...
local svcStarterPack = game:GetService(eEngineServiceSingleton.STARTER_PACK)                                     -- [의미/의도] StarterPack 서비스를 가져옴 ➔ 게임 시작 시 플레이어 인벤토리에 자동으로 활(Tool)을 장착시켜주기 위함
local svcWorkspace = game:GetService(eEngineServiceSingleton.WORKSPACE)                                         -- [의미/의도] Workspace 서비스를 가져옴 ➔ 5일차 과녁 연습장 폴더와 파트를 맵에 추가하기 위함

local fldTargetRange05 = common.createOrReplaceInstance(ePhysical.FOLDER, eLogical.TARGET_RANGE05, svcWorkspace) -- [의미/의도] TargetRange05 Folder 대체 생성 ➔ 기존 과녁판 폴더를 지우고 새로운 5일차 과녁판 경기장을 구성하기 위함

for index = 1, 6 do                                                                                           -- [의미/의도] index 변수를 1부터 6까지 6번 반복 실행 ➔ 쏘아 맞힐 수 있는 6개의 과녁판(Target)을 만들기 위함
    local partTarget = Instance.new(ePhysical.PART)                                        -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 투사체를 맞춰 쓰러뜨릴 과녁판 파트를 생성하기 위함
    partTarget.Name = eLogical.TARGET_PREFIX .. index                                      -- [의미/의도] 과녁판 이름을 인덱스 번호를 붙여 "1_Target" 등으로 설정 ➔ 각 과녁을 개별 번호로 구분하기 위함
    ...
    partTarget.Parent = fldTargetRange05                                                                         -- [의미/의도] 과녁 파트를 TargetRange05 폴더의 자식으로 등록 ➔ 과녁 연습장 폴더 내에 그룹화하여 관리하기 위함
end                                                                                                           -- [의미/의도] 과녁 생성을 위한 반복문(for)의 종료 ➔ 6개의 과녁 생성을 완료함

local toolTrainingBow = common.createOrReplaceInstance(ePhysical.TOOL, eLogical.TRAINING_BOW, svcStarterPack) -- [의미/의도] TrainingBow Tool 대체 생성 ➔ 기존 원거리 무기 활을 지우고 새로운 연습용 활을 초기화하기 위함
toolTrainingBow.ToolTip = "서버에서 투사체를 만드는 연습용 활"                                                                 -- [의미/의도] 활 도구의 마우스 툴팁을 설정 ➔ 플레이어에게 서버 사이드에서 투사체를 생성함을 팁으로 설명하기 위함

local partHandle = Instance.new(ePhysical.PART)                                            -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 캐릭터가 손에 쥘 활의 중심 부위(Handle)를 만들기 위함
partHandle.Name = eLogical.RESERVED_HANDLE                                                 -- [의미/의도] 파트 이름을 반드시 "Handle"로 설정 ➔ 로블록스 도구 탑재 시스템이 캐릭터 손바닥에 부착할 파트로 인식하도록 하기 위함
