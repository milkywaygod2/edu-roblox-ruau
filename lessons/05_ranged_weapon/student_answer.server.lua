...
local serviceDebris = game:GetService(common.enumServiceName.DEBRIS)                                          -- [의미/의도] Debris 서비스를 가져옴 ➔ 월드에 발사된 화살이 무한히 남아 서버에 부하를 주지 않도록 일정 시간 후 자동 소멸시키기 위함
...
toolTrainingBow.Activated:Connect(function()                                                                  -- [의미/의도] 활을 클릭(활성화)했을 때 아래 함수를 실행 ➔ 활 쏘기 발사 로직을 연동하기 위함
    ...
    local partHumanoidRoot = modelCharacter:FindFirstChild(common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 캐릭터 몸통의 중심 파트(HumanoidRootPart)를 찾음 ➔ 화살 발사의 기준점과 바라보는 시선 방향(CFrame)을 구하기 위함
    ...
    local partArrow = Instance.new(common.enumObjectPhysicalType.PART)                                        -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 물리적으로 날아가는 투사체인 화살 객체를 만들기 위함
    partArrow.Name = common.enumObjectLogicalType.PROJECTILE_ARROW_TRAINING                                   -- [의미/의도] 화살 파트 이름을 "TrainingArrow"로 설정 ➔ 충돌 시 이름 비교를 통해 화살인지 식별할 수 있도록 위함
    ...
    serviceDebris:AddItem(partArrow, 6)                                                                       -- [의미/의도] Debris 서비스에 화살을 등록하여 6초 후에 자동 삭제되도록 예약 ➔ 표적에 맞지 않고 허공으로 날아가 떨어진 화살이 맵에 쌓여 렉이 유발되는 것을 방지하기 위함
    ...
    partArrow.Touched:Connect(function(partHit)                                                               -- [의미/의도] 날아가는 화살이 어딘가에 충돌(Touched)했을 때 아래 함수를 실행 ➔ 충돌된 물체를 검사하고 타격 효과를 주거나 화살을 처리하기 위함
        ...
        local humanoidTarget = partHit.Parent:FindFirstChildOfClass(common.enumObjectPhysicalType.HUMANOID)   -- [의미/의도] 부딪힌 대상 부모에서 생명체 속성(Humanoid)을 찾음 ➔ 체력이 존재하는 플레이어나 몬스터인지 판정하기 위함
        if humanoidTarget then humanoidTarget:TakeDamage(DAMAGE) end                                          -- [의미/의도] 생명체가 맞으면 DAMAGE(18)만큼 체력을 깎음 ➔ 원거리 공격 피해를 정량적으로 적용하기 위함
        if common.hasObjectNamePrefix(partHit.Name, common.enumObjectLogicalType.TARGET_PREFIX) then partHit.BrickColor = BrickColor.new("Lime green") end -- [의미/의도] 맞은 파트 이름이 과녁판 enum 접두사("Target_")로 시작한다면 파트 색상을 라임색(Lime green)으로 변경 ➔ TargetPoint 같은 다른 logical type을 오인하지 않고 과녁판 명중만 시각적으로 피드백하기 위함
