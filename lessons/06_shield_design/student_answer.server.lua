...
toolPracticeShield.Equipped:Connect(function()                                                                -- [의미/의도] 도구를 장착(Equipped)했을 때 아래 함수를 실행 ➔ 방패 장착에 따른 캐릭터 능력치 상승(체력 증가, 속도 감소) 효과를 부여하기 위함
    ...
    local humanoidPlayer = modelEquippedCharacter:FindFirstChildOfClass(common.enumObjectPhysicalType.HUMANOID) -- [의미/의도] 캐릭터 내부에서 생명체 정보(Humanoid)를 가져옴 ➔ 플레이어의 체력과 이동 속도 값을 변경하기 위함
...
toolPracticeShield.Unequipped:Connect(function()                                                              -- [의미/의도] 도구 장착을 해제(Unequipped)했을 때 아래 함수를 실행 ➔ 방패 장착으로 변경되었던 스탯을 복구하고 변수를 초기화하기 위함
    ...
    local humanoidPlayer = modelEquippedCharacter:FindFirstChildOfClass(common.enumObjectPhysicalType.HUMANOID) -- [의미/의도] 장착 해제된 캐릭터의 Humanoid 컴포넌트를 가져옴 ➔ 캐릭터 스탯을 원래대로 고치기 위함
...
toolPracticeShield.Handle.Touched:Connect(function(partHit)                                                   -- [의미/의도] 방패 본체(Handle)에 물리적인 파트가 충돌(Touched)했을 때 아래 함수를 실행 ➔ 날아오는 공격 투사체를 방패로 막아내는 로직을 처리하기 위함
    if partHit.Name ~= common.enumObjectLogicalType.PROJECTILE_ARROW_TRAINING and partHit.Name ~= common.enumObjectLogicalType.PROJECTILE_ALL then return end -- [의미/의도] 충돌한 파트 이름이 정확히 연습용 화살 또는 전체 투사체 enum 값이 아니라면 무시 ➔ 이름 일부만 겹치는 다른 logical type을 오인하지 않고 정의된 투사체만 방어 판정하기 위함
    if not modelEquippedCharacter then return end                                                             -- [의미/의도] 방패를 장착한 캐릭터가 존재하지 않는 유령 상태라면 무시 ➔ 비정상적인 충돌 에러를 막기 위함

    local humanoidPlayer = modelEquippedCharacter:FindFirstChildOfClass(common.enumObjectPhysicalType.HUMANOID) -- [의미/의도] 장착자 캐릭터의 Humanoid 정보를 가져옴 ➔ 블로킹 힐(방어 회복)을 처리하기 위함
