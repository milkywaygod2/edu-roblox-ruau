...
toolPracticeShield.Equipped:Connect(function()                                                                -- [의미/의도] 도구를 장착(Equipped)했을 때 아래 함수를 실행 ➔ 방패 장착에 따른 캐릭터 능력치 상승(체력 증가, 속도 감소) 효과를 부여하기 위함
    ...
    local humPlayer = modelEquippedCharacter:FindFirstChildOfClass(ePhysical.HUMANOID) -- [의미/의도] 캐릭터 내부에서 생명체 정보(Humanoid)를 가져옴 ➔ 플레이어의 체력과 이동 속도 값을 변경하기 위함
    if not humPlayer then return end                                                                           -- [의미/의도] Humanoid가 존재하지 않으면 실행 중단 ➔ 캐릭터가 비정상적인 상태이거나 없는 상태에서의 널 에러를 방지함

    tableSavedStats = {MaxHealth = humPlayer.MaxHealth, WalkSpeed = humPlayer.WalkSpeed}                        -- [의미/의도] 장착 직전의 원래 최대 체력과 이동 속도 값을 테이블에 저장 ➔ 방패를 뺐을 때 이 원래 값으로 안전하게 복구하기 위함
    humPlayer.MaxHealth += BONUS_HEALTH                                                                        -- [의미/의도] 캐릭터의 최대 체력을 BONUS_HEALTH(60)만큼 증가시킴 ➔ 방패 장착에 따른 생존율 상승(최대 HP 확장) 효과를 구현하기 위함
    humPlayer.Health = math.min(humPlayer.Health + BONUS_HEALTH, humPlayer.MaxHealth)                            -- [의미/의도] 현재 체력을 BONUS_HEALTH만큼 채워주되, 늘어난 최대 체력을 초과하지 않도록 보정 ➔ 늘어난 최대 HP에 맞춰 현재 체력도 증가시켜 실질적인 방어 상승감을 연출하기 위함
    humPlayer.WalkSpeed -= 2                                                                                   -- [의미/의도] 플레이어의 걷는 속도(WalkSpeed)를 2만큼 감소시킴 ➔ 무거운 철제 방패를 든 대가로 이동 속도가 약간 느려지는 패널티 밸런스를 적용하기 위함
...
toolPracticeShield.Unequipped:Connect(function()                                                              -- [의미/의도] 도구 장착을 해제(Unequipped)했을 때 아래 함수를 실행 ➔ 방패 장착으로 변경되었던 스탯을 복구하고 변수를 초기화하기 위함
    ...
    local humPlayer = modelEquippedCharacter:FindFirstChildOfClass(ePhysical.HUMANOID) -- [의미/의도] 장착 해제된 캐릭터의 Humanoid 컴포넌트를 가져옴 ➔ 캐릭터 스탯을 원래대로 고치기 위함
    if humPlayer then                                                                                          -- [의미/의도] Humanoid가 정상적으로 존재한다면 ➔ 캐릭터가 살아있고 유효할 때만 복구 작업을 수행하기 위함
        humPlayer.MaxHealth = tableSavedStats.MaxHealth                                                        -- [의미/의도] 최대 체력을 저장해두었던 원래 최대 체력으로 원상복구 ➔ 방패 보너스 체력을 제거하기 위함
        humPlayer.Health = math.min(humPlayer.Health, tableSavedStats.MaxHealth)                                -- [의미/의도] 현재 체력이 원래 최대 체력을 초과하지 않도록 보정 ➔ 방패를 해제한 뒤에도 비정상적으로 초과된 체력이 그대로 남아있는 버그를 방지하기 위함
        humPlayer.WalkSpeed = tableSavedStats.WalkSpeed                                                        -- [의미/의도] 걷는 속도를 원래의 정상 속도로 복구 ➔ 속도 저하 패널티를 제거하기 위함
...
toolPracticeShield.Handle.Touched:Connect(function(partHit)                                                   -- [의미/의도] 방패 본체(Handle)에 물리적인 파트가 충돌(Touched)했을 때 아래 함수를 실행 ➔ 날아오는 공격 투사체를 방패로 막아내는 로직을 처리하기 위함
    if partHit.Name ~= eLogical.PROJECTILE_ARROW_TRAINING and partHit.Name ~= eLogical.PROJECTILE_ALL then return end -- [의미/의도] 충돌한 파트 이름이 정확히 연습용 화살 또는 전체 투사체 enum 값이 아니라면 무시 ➔ 이름 일부만 겹치는 다른 logical type을 오인하지 않고 정의된 투사체만 방어 판정하기 위함
    if not modelEquippedCharacter then return end                                                             -- [의미/의도] 방패를 장착한 캐릭터가 존재하지 않는 유령 상태라면 무시 ➔ 비정상적인 충돌 에러를 막기 위함

    local humPlayer = modelEquippedCharacter:FindFirstChildOfClass(ePhysical.HUMANOID) -- [의미/의도] 장착자 캐릭터의 Humanoid 정보를 가져옴 ➔ 블로킹 힐(방어 회복)을 처리하기 위함
    if humPlayer then humPlayer.Health = math.min(humPlayer.Health + BLOCK_HEAL, humPlayer.MaxHealth) end -- [의미/의도] Humanoid가 존재할 경우 체력을 BLOCK_HEAL(5)만큼 회복시키되, 최대 체력을 넘지 않도록 처리 ➔ 적의 투사체를 방패로 막아냈을 때 체력이 일부 채워지는 수비 메리트를 제공하기 위함
