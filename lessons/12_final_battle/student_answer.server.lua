...
local function get_spawn_points()                                                                             -- [의미/의도] 맵에 배치된 스폰 패드 리스트를 반환하는 함수 정의 ➔ 폴더 내에서 동적으로 스폰 패드들을 수집하여 플레이어 이동 처리에 유연하게 대처하기 위함
    ...
    for _, partChild in ipairs(fldFinalBattle12:GetChildren()) do                                               -- [의미/의도] FinalBattle12 폴더의 모든 1단계 자식 객체들을 순회 ➔ 폴더 내부의 스폰 패드들을 전부 찾아내기 위함
        if common.hasEngineLogicalNamePrefix(partChild.Name, eLogical.SPAWN_POINT_PREFIX) and partChild:IsA(ePhysical.BASE_PART) then -- [의미/의도] 이름이 스폰 패드 enum 접두사("SpawnPoint_")로 시작하고 물리 파트(BasePart) 타입이라면 ➔ 다른 장식용 파트나 스크립트를 제외하고 실제 스폰 패드 역할 파트만 정확히 걸러내기 위함
...
local function move_players_to_spawns()                                                                       -- [의미/의도] 모든 접속 중인 플레이어를 팀 스폰 거점으로 순간 이동시키는 함수 정의 ➔ 라운드 개시와 동시에 대전 현장으로 양 팀 플레이어들을 팀별로 분산 배치시키기 위함
    ...
    for index, playerPlayer in ipairs(svcPlayers:GetPlayers()) do                                              -- [의미/의도] 현재 서버에 접속한 모든 플레이어들을 리스트로 받아 순회함 ➔ 전원 한 명도 빠짐없이 텔레포트시키기 위함
        ...
        local partHumanoidRoot = modelCharacter:WaitForChild(eLogical.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 캐릭터 내부에서 중심 몸통 파트(HumanoidRootPart)가 생성될 때까지 안전하게 대기하여 가져옴 ➔ 텔레포트의 기준이 될 물리 몸바닥 중심 부위를 획득하기 위함
...
local function start_round(playerPlayer)                                                                      -- [의미/의도] 라운드를 가동하는 메인 함수 정의 (버튼을 누른 플레이어 정보를 인자로 받음) ➔ 라운드의 상태와 남은 시간 타이머를 관리하고 텔레포트 및 종료 신호를 서버 전역에 설정하기 위함
    ...
    workspace:SetAttribute(eAttrKey.ROUND_STARTER_PLAYER_NAME_RUNTIME, playerPlayer.Name)    -- [의미/의도] Workspace 전역 속성(Attribute)에 "RoundStarter" 이름으로 버튼을 누른 플레이어 이름을 저장 ➔ 누가 이 판을 시작했는지 서버 내 모든 스크립터가 볼 수 있도록 전역 기록하기 위함
    workspace:SetAttribute(eAttrKey.ROUND_STATE, eRoundState.PREPARING)          -- [의미/의도] Workspace 전역 속성 "RoundState"를 "Preparing"(준비 중)으로 설정 ➔ 매치 시작 신호를 보내기 전에 준비 단계 상태임을 클라이언트와 서버가 공유하게 하기 위함
    move_players_to_spawns()                                                                                  -- [의미/의도] move_players_to_spawns 함수를 호출 ➔ 플레이어들을 각자 팀 스폰지로 강제 텔레포트시켜 정렬하기 위함

    workspace:SetAttribute(eAttrKey.ROUND_STATE, eRoundState.PLAYING)            -- [의미/의도] Workspace 전역 속성 "RoundState"를 "Playing"(게임 중)으로 변경 ➔ 이제 전투가 본궤도에 올랐음을 알려 UI 및 스크립터들이 스위치되게 하기 위함
    for intTimeLeft = ROUND_TIME, 0, -1 do                                                                    -- [의미/의도] intTimeLeft 변수를 ROUND_TIME(180초)부터 0초까지 -1씩 감소시키며 루프 실행 ➔ 실시간 남은 시간(초) 타이머 카운트다운을 처리하기 위함
        workspace:SetAttribute(eAttrKey.TIME_LEFT_RUNTIME, intTimeLeft)                      -- [의미/의도] Workspace 전역 속성 "TimeLeft"에 현재 남은 초 단위 시간을 대입 ➔ 플레이어들의 화면(UI)에 실시간 남은 타이머 시간 숫자가 정확하게 업데이트되어 갱신되도록 하기 위함
        task.wait(1)                                                                                          -- [의미/의도] 정확히 1초 동안 스크립트 실행을 일시 지연시킴 ➔ 현실 시간 1초 간격으로 타임아웃을 안전하게 카운트다운하기 위함
    end                                                                                                       -- [의미/의도] 타이머 카운트다운 반복문의 종료 ➔ 180초 타이머 완료

    workspace:SetAttribute(eAttrKey.ROUND_STATE, eRoundState.FINISHED)           -- [의미/의도] Workspace 전역 속성 "RoundState"를 "Finished"(종료됨)로 변경 ➔ 제한시간 종료로 인해 공성 매치가 끝났음을 시스템에 전달하기 위함
