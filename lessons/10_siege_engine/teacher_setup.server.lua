...
local folderSiegeEngine10 = common.createOrReplaceInstance(common.enumObjectPhysicalType.FOLDER, common.enumObjectLogicalType.SIEGE_ENGINE10, serviceWorkspace) -- [의미/의도] SiegeEngine10 Folder 대체 생성 ➔ 기존 공성 맵 폴더를 지우고 새로운 10일차 공성 병기 맵을 구성하기 위함

local partLaunchButton = Instance.new(common.enumObjectPhysicalType.PART)                                     -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 투석기를 작동시킬 물리적 발사 버튼(LaunchButton)을 만들기 위함
partLaunchButton.Name = common.enumObjectLogicalType.LAUNCH_BUTTON                                            -- [의미/의도] 파트 이름을 "LaunchButton"으로 설정 ➔ 성문 공격을 시작할 발사 스위치 파트임을 구분하기 위함
...
partLaunchButton.Parent = folderSiegeEngine10                                                                 -- [의미/의도] 버튼 파트를 SiegeEngine10 폴더의 자식으로 등록 ➔ 10일차 폴더 내부에서 버튼을 관리하기 위함

local clickdetectorButton = Instance.new(common.enumObjectPhysicalType.CLICK_DETECTOR)                        -- [의미/의도] 새로운 클릭 감지기(ClickDetector) 컴포넌트를 생성함 ➔ 마우스로 이 파트를 누를 수 있는 인터랙션 기능을 부여하기 위함
clickdetectorButton.MaxActivationDistance = 24                                                                -- [의미/의도] 마우스 클릭 작동 한계 거리를 24칸으로 설정 ➔ 지나치게 멀리서 저격하듯 스위치를 작동시키는 꼼수 플레이를 차단하기 위함
clickdetectorButton.Parent = partLaunchButton                                                                 -- [의미/의도] 클릭 감지기의 부모를 LaunchButton 파트로 설정 ➔ 해당 버튼 파트에 클릭 리스너 기능을 심기 위함

local partLaunchPoint = Instance.new(common.enumObjectPhysicalType.PART)                                      -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 공성 돌이 발사되어 출발하는 물리적 위치 마커(LaunchPoint)를 만들기 위함
partLaunchPoint.Name = common.enumObjectLogicalType.LAUNCH_POINT                                              -- [의미/의도] 파트 이름을 "LaunchPoint"로 지정 ➔ 스크립트가 돌 탄환을 스폰시킬 지점으로 참조하게 하기 위함
...
partLaunchPoint.Parent = folderSiegeEngine10                                                                  -- [의미/의도] 마커 파트를 SiegeEngine10 폴더의 자식으로 등록 ➔ 10일차 폴더 내에서 관리하기 위함

local partTargetPoint = Instance.new(common.enumObjectPhysicalType.PART)                                      -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 탄환이 날아가 떨어질 목표물 위치 마커(TargetPoint)를 만들기 위함
partTargetPoint.Name = common.enumObjectLogicalType.TARGET_POINT                                              -- [의미/의도] 파트 이름을 "TargetPoint"로 설정 ➔ 탄환 발사 궤적을 계산할 종점 타겟 좌표로 쓰기 위함
...
partTargetPoint.Parent = folderSiegeEngine10                                                                  -- [의미/의도] 타겟 파트를 SiegeEngine10 폴더의 자식으로 등록 ➔ 10일차 폴더 내에 함께 묶어 관리하기 위함
