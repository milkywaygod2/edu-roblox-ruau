...
local serviceDebris = game:GetService(common.enumServiceName.DEBRIS)                                          -- [의미/의도] Debris 서비스를 가져옴 ➔ 월드 상에 발사된 공성돌 탄환이 일정 시간 후에 알아서 소멸되도록 예약하고 관리하기 위함
local folderSiegeEngine10 = workspace:WaitForChild(common.enumObjectLogicalType.SIEGE_ENGINE10)               -- [의미/의도] Workspace에서 "SiegeEngine10" 폴더가 생성될 때까지 대기하여 가져옴 ➔ 10일차 투석기 오브젝트들이 완전히 로드된 후 스크립트를 수행하기 위함
local partLaunchButton = folderSiegeEngine10:WaitForChild(common.enumObjectLogicalType.LAUNCH_BUTTON)         -- [의미/의도] 10일차 폴더 내부에서 "LaunchButton" 파트가 생성될 때까지 대기하여 가져옴 ➔ 발사를 작동시킬 입력 장치(버튼)를 정확하게 셋업하기 위함
local partLaunchPoint = folderSiegeEngine10:WaitForChild(common.enumObjectLogicalType.LAUNCH_POINT)           -- [의미/의도] 10일차 폴더 내부에서 "LaunchPoint" 파트가 생성될 때까지 대기하여 가져옴 ➔ 공성 돌이 소환되는 공중 시작 위치 좌표를 가져오기 위함
local partTargetPoint = folderSiegeEngine10:WaitForChild(common.enumObjectLogicalType.TARGET_POINT)           -- [의미/의도] 10일차 폴더 내부에서 "TargetPoint" 파트가 생성될 때까지 대기하여 가져옴 ➔ 투사체가 날아가 부딪힐 정면 목표물 위치 좌표를 가져오기 위함
...
local function launch_stone(player)                                                                           -- [의미/의도] 공성 돌을 소환하여 발사하는 함수 정의 (버튼을 누른 플레이어 정보를 인자로 받음) ➔ 버튼 입력 시 정교한 비행 궤적을 가지는 돌 투사체를 생성하기 위함
    ...
    local partSiegeStone = Instance.new(common.enumObjectPhysicalType.PART)                                   -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 성문에 타격을 가할 무거운 공성 탄환(SiegeStone) 객체를 만들기 위함
    partSiegeStone.Name = common.enumObjectLogicalType.SIEGE_STONE                                            -- [의미/의도] 탄환 파트 이름을 "SiegeStone"으로 설정 ➔ 성벽이나 성문이 부딪혔을 때 공성 탄환에 의한 충돌인지 식별할 수 있게 하기 위함
    ...
    serviceDebris:AddItem(partSiegeStone, 8)                                                                  -- [의미/의도] 돌 탄환을 Debris 서비스에 등록하여 8초 후에 자동으로 소멸하도록 예약 ➔ 타격을 못하고 맵 밖에 떨어진 대포알이 영구히 남아 렉을 거는 현상을 방지하기 위함
