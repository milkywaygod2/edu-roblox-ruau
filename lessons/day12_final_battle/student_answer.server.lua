...
local folderDay12FinalBattle = workspace:WaitForChild("Day12_FinalBattle")           -- [의미/의도] Workspace에서 "Day12_FinalBattle" 폴더가 생성될 때까지 대기하여 가져옴 ➔ 12일차 셋업 오브젝트가 완전히 로드된 후 스크립트를 동작시키기 위함
local partRoundStartButton = folderDay12FinalBattle:WaitForChild("RoundStartButton") -- [의미/의도] 12일차 폴더 내부에서 "RoundStartButton" 파트가 생성될 때까지 대기하여 가져옴 ➔ 클릭 감지기 클릭 이벤트를 연결하기 위함
...
local function get_spawn_points()                                                -- [의미/의도] 맵에 배치된 스폰 패드 리스트를 반환하는 함수 정의 ➔ 폴더 내에서 동적으로 스폰 패드들을 수집하여 플레이어 이동 처리에 유연하게 대처하기 위함
    ...
    for _, partChild in ipairs(folderDay12FinalBattle:GetChildren()) do          -- [의미/의도] Day12_FinalBattle 폴더의 모든 1단계 자식 객체들을 순회 ➔ 폴더 내부의 스폰 패드들을 전부 찾아내기 위함
