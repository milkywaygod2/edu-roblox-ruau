...
local folderOld = serviceWorkspace:FindFirstChild("Day12_FinalBattle") -- [의미/의도] Workspace에서 기존 "Day12_FinalBattle" 폴더가 존재하는지 확인 ➔ 이전 수업 찌꺼기가 있는지 찾기 위함
if folderOld then folderOld:Destroy() end                              -- [의미/의도] 기존 폴더가 존재한다면 제거 ➔ 12일차 셋업 재실행 시 라운드 개시 버튼과 스폰 지점들이 겹쳐서 생기는 에러를 방지하기 위함

local folderDay12FinalBattle = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder) 객체를 생성함 ➔ 12일차 실습에서 관리할 라운드 매니저 버튼과 스폰 패드를 하나로 묶기 위함
folderDay12FinalBattle.Name = "Day12_FinalBattle"     -- [의미/의도] 폴더 이름을 "Day12_FinalBattle"로 설정 ➔ 탐색기에서 12일차 공성 대대전 영역임을 파악하기 위함
folderDay12FinalBattle.Parent = serviceWorkspace      -- [의미/의도] 폴더 부모를 Workspace로 설정 ➔ 폴더가 월드 상에 존재하도록 만들기 위함
...
partRoundStartButton.Parent = folderDay12FinalBattle           -- [의미/의도] 버튼 파트를 Day12_FinalBattle 폴더의 자식으로 등록 ➔ 12일차 폴더 내부에서 버튼을 관리하기 위함
...
for index, x in ipairs({-28, 28}) do                                                         -- [의미/의도] -28과 28 두 X좌표 값을 가지고 index 1, 2로 나누어 2번 반복 실행 ➔ 성의 양 측면에 각각 청팀과 홍팀이 부활할 스폰 거점 위치를 다르게 계산하여 소환하기 위함
    ...
    partSpawnPoint.Name = "SpawnPoint_" .. index                                             -- [의미/의도] 파트 이름을 고유 번호를 붙여 "SpawnPoint_1" 등으로 설정 ➔ 각각의 팀 스폰 패드를 구분하기 위함
    ...
    partSpawnPoint.Parent = folderDay12FinalBattle                                           -- [의미/의도] 스폰 패드 파트를 Day12_FinalBattle 폴더의 자식으로 등록 ➔ 12일차 폴더 내에 함께 묶어 보관하기 위함
