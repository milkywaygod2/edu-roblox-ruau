...
-- 수업: rock_tool_01 - 돌멩이 디자인과 기초 무기
...
-- 역할: teacher_setup.server.lua, 선생님 전용 수업 준비 코드입니다.
-- 편집 위치: Roblox Studio에서 Explorer, Properties, Output 창을 켜고 작업합니다.
-- 붙여넣기 위치: ServerScriptService > Script 이름 rock_tool_TeacherSetup_01
...
local folderOldArena = serviceWorkspace:FindFirstChild("Arena01") -- [의미/의도] 기존 Arena01 폴더를 찾음 ➔ 중복 생성을 방지하기 위한 대상을 찾기 위함
if folderOldArena then folderOldArena:Destroy() end               -- [의미/의도] 기존 폴더가 있다면 삭제함 ➔ 중복 실행 시 맵이 꼬이거나 찌꺼기가 남는 것을 막기 위함

local folderArena01 = Instance.new("Folder") -- [의미/의도] 새 폴더 객체를 생성함 ➔ 1일차 맵 구성 요소들을 한곳에 깔끔하게 묶어 관리하기 위함
folderArena01.Name = "Arena01"               -- [의미/의도] 폴더 이름을 "Arena01"로 설정 ➔ 탐색기에서 이름을 식별할 수 있도록 함
folderArena01.Parent = serviceWorkspace      -- [의미/의도] 폴더의 부모를 Workspace로 지정 ➔ 게임 세상(Workspace)에 폴더를 배치하기 위함
