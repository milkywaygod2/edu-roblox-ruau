...
local serviceDebris = game:GetService("Debris") -- [의미/의도] Debris 서비스를 가져옴 ➔ 생성된 투사체를 일정 시간 뒤 자동으로 삭제하기 위함
...
toolPracticeRock.Activated:Connect(function() -- [의미/의도] 도구를 클릭(활성화)했을 때 아래 함수를 실행 ➔ 마우스 클릭 시 무기가 동작하도록 이벤트를 연결함
    ...
    local partHumanoidRoot = modelCharacter:FindFirstChild("HumanoidRootPart") -- [의미/의도] 캐릭터 몸통의 중심 파트를 찾음 ➔ 발사 위치와 시선을 알아내기 위함
    ...
    local partRock = Instance.new("Part")                                                                           -- [의미/의도] 날아갈 파트(돌멩이)를 생성함 ➔ 물리적으로 날아가는 투사체 객체를 생성함
    partRock.Name = "ThrownPracticeRock"                                                                            -- [의미/의도] 파트 이름을 "ThrownPracticeRock"으로 설정 ➔ 충돌 시 이름으로 구분할 수 있도록 함
    ...
    partRock.Touched:Connect(function(partHit) -- [의미/의도] 돌멩이가 어딘가에 부딪혔을 때(Touched) 아래 함수를 실행 ➔ 적과 부딪혔는지 감지하기 위함
        ...
        local modelTarget = partHit:FindFirstAncestorOfClass("Model")                         -- [의미/의도] 충돌한 파트가 속한 최상위 모델을 찾음 ➔ 부딪힌 대상이 캐릭터(Model)인지 검사하기 위함
        local humanoidTarget = modelTarget and modelTarget:FindFirstChildOfClass("Humanoid")  -- [의미/의도] 그 모델 안에 생명체(Humanoid)가 있는지 찾음 ➔ 체력을 깎을 수 있는 대상인지 식별하기 위함
        local partTargetRoot = modelTarget and modelTarget:FindFirstChild("HumanoidRootPart") -- [의미/의도] 타격 대상의 중심 파트를 찾음 ➔ 넉백을 주기 위해 몸통을 찾기 위함
