...
partGridBase.Name = eLogical.GRID_BASE                                                      -- [의미/의도] 파트 이름을 "GridBase"로 설정 ➔ 바닥 구역임을 탐색기에서 구분할 수 있게 지정함
...
partGridBase.Parent = fldCoverField02                                                                             -- [의미/의도] 바닥 파트를 CoverField02 폴더의 자식으로 등록 ➔ 해당 폴더 안에서 깔끔하게 관리하기 위함

for index = 1, 5 do                                                                                           -- [의미/의도] index 변수를 1부터 5까지 증가시키며 반복함 ➔ 총 5개의 엄폐물 배치 기준 표시판(Marker)을 만들기 위함
    local partCoverMarker = Instance.new(ePhysical.PART)                                    -- [의미/의도] 새로운 파트(Part)를 생성함 ➔ 엄폐물을 배치할 기준 위치 마커를 만들기 위함
    partCoverMarker.Name = eLogical.COVER_MARKER_PREFIX .. index                            -- [의미/의도] 마커의 이름을 번호를 붙여 "1_CoverMarker" 등으로 설정 ➔ 각각의 마커를 이름으로 쉽게 구분하기 위함
    ...
    partCoverMarker.Parent = fldCoverField02                                                                      -- [의미/의도] 마커 파트를 CoverField02 폴더의 자식으로 등록 ➔ 2일차 폴더 내부에서 엄폐물 마커들을 한데 모아 관리하기 위함
