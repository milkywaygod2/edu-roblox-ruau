...
local folderDay08Castle = workspace:WaitForChild("Day08_Castle") -- [의미/의도] Workspace 내에서 "Day08_Castle" 폴더가 생성될 때까지 대기하여 가져옴 ➔ 8일차 성 오브젝트가 로드된 후 스크립트를 안정적으로 진행하기 위함
local modelGate = folderDay08Castle:WaitForChild("Gate")         -- [의미/의도] 8일차 성 폴더 내부에서 "Gate" 성문 모델이 생성될 때까지 대기하여 가져옴 ➔ 성문에 타격 및 파괴 판정을 걸 대상 모델을 확실히 참조하기 위함
