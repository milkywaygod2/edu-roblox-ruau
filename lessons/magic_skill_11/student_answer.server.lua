...
local servicePlayers = game:GetService("Players")                               -- [의미/의도] Players 서비스를 가져옴 ➔ 게임을 떠난 플레이어 정보를 감지하고 쿨타임 데이터를 청소하기 위함
local serviceReplicatedStorage = game:GetService("ReplicatedStorage")           -- [의미/의도] ReplicatedStorage 서비스를 가져옴 ➔ 클라이언트와 통신하기 위해 공유된 통신망에서 리모트 이벤트를 참조하기 위함
local remoteeventCastMagic = serviceReplicatedStorage:WaitForChild("CastMagic") -- [의미/의도] ReplicatedStorage에서 "CastMagic" 리모트 이벤트가 생성될 때까지 대기하여 가져옴 ➔ 클라이언트의 마법 시전 요청 신호를 서버에서 대기하고 수신하기 위함
...
local function cast_magic(player, targetPosition)          -- [의미/의도] 클라이언트의 요청을 받아 마법을 실행하는 서버 함수 정의 (요청 플레이어, 마우스 좌표 전달 받음) ➔ 보안이 유지되는 서버에서 실제 폭발을 소환하고 범위를 연산하기 위함
    ...
    local partHumanoidRoot = modelCharacter and modelCharacter:FindFirstChild("HumanoidRootPart") -- [의미/의도] 캐릭터 몸통의 중심 파트(HumanoidRootPart)를 찾음 ➔ 사거리 검증을 위한 플레이어의 현재 실제 월드 좌표를 확인하기 위함
    ...
    local explosionMagic = Instance.new("Explosion") -- [의미/의도] 새로운 폭발(Explosion) 객체를 생성함 ➔ 시각적으로 멋지게 터지는 폭풍 이펙트와 물리 충격을 보여주기 위함
    ...
    for _, object in ipairs(workspace:GetDescendants()) do                                                                              -- [의미/의도] 게임 내의 모든 하위 오브젝트들을 전부 순회함 ➔ 폭발 반경 내에 들어온 타격 대상을 일일이 감지하기 위함
        if object:IsA("Humanoid") then                                                                                                  -- [의미/의도] 순회 객체가 생명체 속성(Humanoid)인 경우 ➔ 폭발 범위 데미지를 입을 생명체를 식별하기 위함
            ...
            local partTargetRoot = modelTarget and modelTarget:FindFirstChild("HumanoidRootPart")                                       -- [의미/의도] 대상의 중심 몸통 파트(HumanoidRootPart)를 찾음 ➔ 폭발 중심점과의 실제 거리를 계산해내기 위함
