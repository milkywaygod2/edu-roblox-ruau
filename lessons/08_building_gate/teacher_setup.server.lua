...
local serviceWorkspace = game:GetService(common.enumServiceName.WORKSPACE)                                    -- [의미/의도] Workspace 서비스를 가져옴 ➔ 맵 상에 8일차 성문 폴더와 관련 파트들을 생성하고 배치하기 위함
local folderCastle08 = common.createOrReplaceInstance(common.enumObjectPhysicalType.FOLDER, common.enumObjectLogicalType.CASTLE08, serviceWorkspace) -- [의미/의도] Castle08 Folder 대체 생성 ➔ 기존 성벽/성문 폴더를 지우고 새로운 8일차 성문 실습장을 구성하기 위함

local modelGate = Instance.new(common.enumObjectPhysicalType.MODEL)                                           -- [의미/의도] 새로운 모델(Model) 객체를 생성함 ➔ 성문을 구성하는 여러 개의 나무판자 파트들을 하나의 성문 모델로 그룹화하기 위함
modelGate.Name = common.enumObjectLogicalType.GATE                                                            -- [의미/의도] 모델 이름을 "Gate"로 설정 ➔ 학생 스크립트가 해당 이름("Gate")으로 성문 모델을 찾아 작동시키게 하기 위함
...
for index = 1, 5 do                                                                                           -- [의미/의도] index 변수를 1부터 5까지 5번 반복 실행 ➔ 성문을 이루는 5개의 개별 나무판자 파트를 나란히 배치하기 위함
    local partGatePlank = Instance.new(common.enumObjectPhysicalType.PART)                                    -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 성문의 한 칸을 구성할 세로형 나무판자 파트를 만들기 위함
    partGatePlank.Name = common.enumObjectLogicalType.GATE_PLANK_PREFIX .. index                              -- [의미/의도] 파트 이름을 인덱스 번호를 붙여 "1_GatePlank" 등으로 설정 ➔ 각 판자 파트를 고유 번호로 쉽게 구분하기 위함
