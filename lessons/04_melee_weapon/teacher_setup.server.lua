...
local partHandle = Instance.new(ePhysical.PART)                                            -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 도구가 손에 쥐어질 때 기준점이 되는 부위(Handle)를 만들기 위함
partHandle.Name = eLogical.RESERVED_HANDLE                                                 -- [의미/의도] 파트 이름을 반드시 "Handle"로 설정 ➔ 로블록스 도구 시스템이 캐릭터의 손에 붙이는 부위로 자동 인식하게 하기 위함
...
local fldDummies04 = common.createOrReplaceInstance(ePhysical.FOLDER, eLogical.DUMMIES04, svcWorkspace) -- [의미/의도] Dummies04 Folder 대체 생성 ➔ 기존 더미 그룹 폴더를 삭제하고 연습용 더미들을 새로 준비하기 위함

for index = 1, 5 do                                                                                           -- [의미/의도] index 변수를 1부터 5까지 증가시키며 반복함 ➔ 총 5마리의 연습용 공격 대상 더미를 정렬하여 소환하기 위함
    local modelPracticeDummy = Instance.new(ePhysical.MODEL)                                -- [의미/의도] 새로운 모델(Model) 객체를 생성함 ➔ 캐릭터 역할을 수행할 파트와 Humanoid를 하나로 그룹화하기 위함
    modelPracticeDummy.Name = eLogical.COOLDOWN_DUMMY_PREFIX .. index                       -- [의미/의도] 모델 이름을 인덱스 번호를 붙여 "1_CooldownDummy" 등으로 설정 ➔ 여러 더미들을 각각 고유 번호로 쉽게 구분하기 위함
    modelPracticeDummy.Parent = fldDummies04                                                                     -- [의미/의도] 모델 부모를 Dummies04 폴더로 설정 ➔ 4일차 더미 폴더 내에 배치하여 소속을 정리하기 위함

    local partHumanoidRoot = Instance.new(ePhysical.PART)                                   -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 더미 캐릭터의 중심 몸통(HumanoidRootPart) 역할을 할 파트를 만들기 위함
    partHumanoidRoot.Name = eLogical.RESERVED_HUMANOID_ROOT_PART                            -- [의미/의도] 파트 이름을 반드시 "HumanoidRootPart"로 설정 ➔ 로블록스 Humanoid 시스템이 캐릭터 모델의 중심 기준점으로 자동 인식하게 하기 위함
    ...
    local humPractice = Instance.new(ePhysical.HUMANOID)                                   -- [의미/의도] 새로운 Humanoid 객체를 생성함 ➔ 일반 파트에 생명력(HP)과 피해(TakeDamage)를 입을 수 있는 생명체 특성을 부여하기 위함
    humPractice.Parent = modelPracticeDummy                                                                      -- [의미/의도] Humanoid 객체를 더미 모델의 자식으로 등록 ➔ 더미 모델이 실제 HP 바를 가진 살아있는 타겟 dummy가 되도록 활성화하기 위함
