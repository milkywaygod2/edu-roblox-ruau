...
- `lessons/ServerScriptService/Student*Designs/*_student_answer.luau`: 회차별 학생용 디자인 ModuleScript입니다. `StudentRockDesigns`, `StudentBrickDesigns`, `StudentSwordDesigns`처럼 개념별 폴더를 쓰며, 각 답안은 반별 폴더(`Dobong`/`Nowon`/`Example`) 아래에 있습니다. `TeacherSetup`의 `ACTIVE_CLASS`로 지정한 반의 폴더만 적용되며 다른 반은 없는 것처럼 무시됩니다. 허용된 데이터 값만 수정하고, 실제 아이템 생성, 획득, 데미지, 자원, 쿨타임, 라운드 판정은 `Common`이 처리합니다.
...
5. Explorer에서 `ReplicatedStorage/Common`, `ServerScriptService/TeacherSetup`, `StudentRockDesigns/<반>`, `StudentBrickDesigns/<반>`, `StudentSwordDesigns/<반>` 같은 `Student*Designs/<반>` 폴더가 만들어졌는지 확인합니다. (반 = `ACTIVE_CLASS` 값에 해당하는 폴더)
...
2회차 벽돌은 `ServerScriptService > StudentBrickDesigns` 폴더 안의 학생 ModuleScript가 1회차 돌멩이와 같은 데이터 table을 `return`합니다. 학생은 벽돌의 `Material`, `Size`, `CollisionShape`, `CollisionShapeMode`, `LookShape`, `SpawnCount`를 조정하고, Common은 물리 벽돌 생성, 등에 메기, `Q` 내려놓기, 충돌과 피해 판정을 처리합니다.

3~12회차도 `ServerScriptService > StudentResourceWallDesigns`, `StudentSwordDesigns`, `StudentBowDesigns` 같은 전용 폴더 안의 ModuleScript가 설정 table만 `return`합니다. `Common`은 파일명 숫자가 아니라 폴더 매핑으로 회차와 개념을 판단합니다. 학생 파일은 `require(Common)`이나 `common.install*`을 호출하지 않고, `TeacherSetup`이 Play 시점에 이 ModuleScript들을 읽어 설치합니다.

`Appearance`는 돌의 모양 묶음입니다. `Size`는 실제 게임 기준 크기이자 `LookShape`가 들어갈 최대 크기입니다. `CollisionShape`는 기본 충돌 Part 모양이고, `CollisionShapeMode`는 자동 검토할지(`"Auto"`, 기본값), 그 모양을 그대로 쓸지(`"Shape"`), 룩의 바운딩 박스로 맞출지(`"LookBounds"`), 단일 MeshPart 후보를 검토할지(`"LookMesh"`)를 고르는 문자열입니다. `LookShape`는 그 크기 안에 맞춰 붙는 겉모습 모델입니다. (학생들이 엉뚱한 색상을 임의로 지정해 비주얼을 해치지 못하도록, 서버 측에서 재질에 부합하는 고유 기본 색상이 자동으로 강제 적용됩니다.)
...
`Common`은 `RockLooks/Meteor`를 찾아 Handle과 투사체에 겉모습으로 붙입니다. `LookShape` 모델은 `Size` 안으로 맞춰지고, 내부 Script/ModuleScript/LocalScript는 제거됩니다. 기본값에서는 충돌과 데미지 판정을 안전한 기본 Part가 맡습니다. 룩 기준으로 바닥 접촉을 더 맞추고 싶으면 `appearance.CollisionShapeMode = "LookBounds"` 또는 `"Auto"`를 선택합니다.
...
| 2회차 학생 벽돌 데이터 | `ServerScriptService` | `lessons/ServerScriptService/StudentBrickDesigns/02_student_answer.luau` |
| 3~12회차 학생 디자인 데이터 | `ServerScriptService` | `lessons/ServerScriptService/StudentResourceWallDesigns/03_student_answer.luau`, `StudentSwordDesigns/04_student_answer.luau` 등 전용 폴더 |
...
| 02 | 물리 벽돌 쌓기/엄폐물 | `OutpostBattleWorld/Battlefield/CoverMarker_*`, `ReplicatedStorage/DropCarriedBlock`, `ServerScriptService/StudentBrickDesigns`, `StackBrick_*` | `StudentBrickDesigns/02_student_answer.luau` |
| 03 | 자원 기반 방벽 | `OutpostBattleWorld/BuildArea/BuildButton`, `WallSpawn`, `ServerScriptService/StudentResourceWallDesigns` | `StudentResourceWallDesigns/03_student_answer.luau` |
| 04 | 근접 무기/디바운스 | `ObjectiveArea/DuelGuard_*`, `ItemSpawn_FieldSword_*`, `ServerScriptService/StudentSwordDesigns` | `StudentSwordDesigns/04_student_answer.luau` |
| 05 | 원거리 무기 | `ObjectiveArea/RangeTarget_*`, `ItemSpawn_FieldBow_*`, `ServerScriptService/StudentBowDesigns` | `StudentBowDesigns/05_student_answer.luau` |
| 06 | 방패 | `ItemSpawn_FieldShield_*`, `ServerScriptService/StudentShieldDesigns` | `StudentShieldDesigns/06_student_answer.luau` |
| 07 | 갑옷 | `ItemSpawn_FieldArmor_*`, `ServerScriptService/StudentArmorDesigns` | `StudentArmorDesigns/07_student_answer.luau` |
| 08 | 파괴되는 문 | `OutpostBattleWorld/Fortification/Gate`, `ServerScriptService/StudentGateDesigns` | `StudentGateDesigns/08_student_answer.luau` |
| 09 | 부분 파괴 석벽 | `OutpostBattleWorld/Fortification/StoneWall`, `ServerScriptService/StudentStoneWallDesigns` | `StudentStoneWallDesigns/09_student_answer.luau` |
| 10 | 중장비 | `OutpostBattleWorld/SiegeEngine/LaunchButton`, `LaunchPoint`, `TargetPoint`, `ServerScriptService/StudentSiegeEngineDesigns` | `StudentSiegeEngineDesigns/10_student_answer.luau` |
| 11 | 마법 스킬 | `ItemSpawn_MagicStaff_*`, `ObjectiveArea/ArcaneGuard_*`, `ReplicatedStorage/CastMagic`, `ServerScriptService/StudentMagicDesigns` | `StudentMagicDesigns/11_student_answer.luau`, `StarterPlayerScripts/11_student_answer.local.luau` |
| 12 | 최종 5v5 공방전 | `Battlefield/RoundStartButton`, `SpawnPoint_Blue`, `SpawnPoint_Red`, `Teams/Blue`, `Teams/Red`, `ServerScriptService/StudentFinalBattleDesigns` | `StudentFinalBattleDesigns/12_student_answer.luau` |
...
- Tool 클릭이 반응하지 않음: `TeacherSetup`으로 `ItemSpawn_*` 마커가 만들어졌고, 해당 회차 ModuleScript가 `StudentSwordDesigns`, `StudentBowDesigns` 같은 전용 폴더에 들어 있는지 확인합니다. 1회차 돌멩이는 `StudentRockDesigns`, 2회차 벽돌은 `StudentBrickDesigns` ModuleScript를 `TeacherSetup`이 읽어 설치합니다.
- 같은 학생 장비가 겹침: 1회차 돌멩이와 2회차 벽돌은 각 디자인 폴더 안에 학생별 ModuleScript가 있는지 확인합니다. 다른 Tool 회차는 학생별 `VariantId` 또는 `SpawnOffset`을 다르게 설정합니다.
