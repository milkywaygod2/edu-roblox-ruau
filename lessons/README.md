...
이 가이드는 로블록스 스튜디오에서 공통 모듈 라이브러리(`Common`)를 올바르게 구성하고, 1일차 수업(돌 투척 및 물리 자원 획득)을 세팅하여 검증하는 단계별 실전 가이드입니다. Rojo 등의 외부 동기화 도구 없이 수동 복사-붙여넣기 방식으로 환경을 조성할 때 발생할 수 있는 참조 오류를 예방하기 위해 작성되었습니다.

---

### 1단계. 필수 뷰(View) 패널 및 개발 환경 준비
1. 로블록스 스튜디오 상단 메뉴의 **View** 탭을 클릭합니다.
2. 개발 및 디버깅을 위해 다음 3개의 창을 활성화하여 적절하게 배치합니다:
   - **Explorer** (오브젝트 탐색기)
   - **Properties** (속성 창)
   - **Output** (출력 창)
> [!NOTE]
> **Output(출력) 창**은 Luau 컴파일 오류, 모듈 로딩(Require) 실패 로그, 학생 코드 밸리데이션 로그를 실시간으로 모니터링하는 핵심 디버깅 도구이므로 항상 열어두어야 합니다.

---

### 2단계. 공통 라이브러리 및 패키지 환경 세팅 (Common)
로블록스 스튜디오는 로컬의 파일 계층 구조와 다르게 **오브젝트의 물리적 트리(부모-자식 관계) 계층**을 기준으로 작동합니다. `Common.lua`는 하위 15개 서브 모듈을 동적으로 가져오므로, 반드시 아래 구조와 같이 부모-자식 인스턴스 관계를 일치시켜야 합니다.

#### 1. Roblox 인스턴스 타입 및 파일 매핑 규칙
*   **`.lua` 파일 / `.module.lua` 파일**: 로블록스 스튜디오에서는 **`ModuleScript`** 인스턴스로 변환되어야 합니다.
*   **`.server.lua` 파일**: 로블록스 스튜디오에서는 일반 **`Script`** 인스턴스로 변환되어야 합니다.
*   **`.client.lua` 파일**: 로블록스 스튜디오에서는 **`LocalScript`** 인스턴스로 변환되어야 합니다.
*   > [!IMPORTANT]
    > 로블록스 내에서 require 참조가 가능하려면 인스턴스 이름에 확장자(`.lua`, `.module.lua` 등)를 포함하지 않고 **순수 파일명만** 이름으로 지정해야 합니다. (예: `Common.lua` -> 이름 `Common`)

#### 2. 에셋 물리 계층 트리 구조 (Asset Hierarchy Tree)
아래의 구조도와 완벽하게 일치하도록 **Explorer** 창에 인스턴스를 배치하고 각 로컬 파일 내용을 붙여넣으십시오.

```
ReplicatedStorage (서비스)
└── Common (ModuleScript)  <-- [Common.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common.lua) 내용 복사
    ├── Appearance (ModuleScript)  <-- [Common/Appearance.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/Appearance.lua) 내용 복사
    ├── BuildSystems (ModuleScript)  <-- [Common/BuildSystems.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/BuildSystems.lua) 내용 복사
    ├── CombatRules (ModuleScript)  <-- [Common/CombatRules.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/CombatRules.lua) 내용 복사
    ├── CoreBlock (ModuleScript)  <-- [Common/CoreBlock.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/CoreBlock.lua) 내용 복사
    ├── CoreEnums (ModuleScript)  <-- [Common/CoreEnums.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/CoreEnums.lua) 내용 복사
    ├── CurriculumSetup (ModuleScript)  <-- [Common/CurriculumSetup.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/CurriculumSetup.lua) 내용 복사
    ├── Effect (ModuleScript)  <-- [Common/Effect.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/Effect.lua) 내용 복사
    ├── EngineEnsure (ModuleScript)  <-- [Common/EngineEnsure.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/EngineEnsure.lua) 내용 복사
    ├── FieldItem (ModuleScript)  <-- [Common/FieldItem.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/FieldItem.lua) 내용 복사
    ├── FinalBattle (ModuleScript)  <-- [Common/FinalBattle.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/FinalBattle.lua) 내용 복사
    ├── Fortification (ModuleScript)  <-- [Common/Fortification.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/Fortification.lua) 내용 복사
    ├── MagicSystem (ModuleScript)  <-- [Common/MagicSystem.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/MagicSystem.lua) 내용 복사
    ├── SiegeEngine (ModuleScript)  <-- [Common/SiegeEngine.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/SiegeEngine.lua) 내용 복사
    ├── StudentConfig (ModuleScript)  <-- [Common/StudentConfig.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/StudentConfig.lua) 내용 복사
    ├── ThrowingStone (ModuleScript)  <-- [Common/ThrowingStone.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/ThrowingStone.lua) 내용 복사
    └── WeaponSystems (ModuleScript)  <-- [Common/WeaponSystems.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/WeaponSystems.lua) 내용 복사
```

#### 3. 세부 수동 임포트 및 구성 순서
1. **부모 ModuleScript 생성**:
   - 스튜디오 **Explorer** 창에서 `ReplicatedStorage`를 우클릭하고 **Insert Object > ModuleScript**를 클릭하여 모듈을 만듭니다.
   - 생성된 모듈의 이름을 `Common`으로 변경합니다.
   - [Common.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common.lua) 파일의 소스 코드를 복사하여 이 모듈 안에 붙여넣습니다.
2. **자식 ModuleScript들 생성 및 종속**:
   - 위에서 만든 `Common` 인스턴스를 마우스 우클릭하고 **Insert Object > ModuleScript**를 실행합니다.
   - 생성된 자식 모듈의 이름을 `Appearance`로 변경합니다. (반드시 `Common` 아래에 자식 구조로 들어가야 합니다!)
   - [Common/Appearance.lua](file:///E:/_Develop/edu/edu-roblox-ruau/lessons/Common/Appearance.lua) 내용을 복사해 붙여넣습니다.
   - 이 과정을 반복하여 나머지 14개 서브 모듈(`BuildSystems`부터 `WeaponSystems`까지)을 모두 `Common`의 자식 인스턴스로 생성하고 각각의 소스 코드를 붙여넣습니다.
3. **참조 무결성 주의 사항**:
   - `Common.lua`는 `local Appearance = require(script.Appearance)`와 같이 `script` 내부 경로를 찾습니다. 자식 모듈들이 `Common` 하위에 제대로 종속되어 있지 않고 `ReplicatedStorage`에 병렬로 풀려 있는 경우, 로블록스는 `Appearance is not a valid member of ModuleScript "Common"` 오류를 던지고 전체 시스템 작동을 멈추게 됩니다.

---

### 3단계. 1일차 교사용 월드 세팅 (Teacher Setup)
1. **스크립트 생성**:
   - 스튜디오의 **Explorer** 창에서 `ServerScriptService`를 마우스 우클릭하고 **Insert Object > Script**를 생성합니다.
   - 스크립트 이름을 `TeacherSetup`으로 변경합니다. (일반 `Script` 타입인지 확인하십시오. `ModuleScript`가 아닙니다.)
2. **코드 복사**:
...
### 4단계. 학생 모범답안 적용 및 물리 루프 검증 (Student Answer)
1. **학생 모듈 배치**:
