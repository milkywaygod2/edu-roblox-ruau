...
-- 수업: building_gate_08 - 건물과 파괴되는 성문
...
-- 선행 조건: 선생님이 Workspace/Castle08/Gate를 먼저 만들어야 합니다.
...
local folderCastle08 = workspace:WaitForChild("Castle08") -- [의미/의도] Workspace 내에서 "Castle08" 폴더가 생성될 때까지 대기하여 가져옴 ➔ 8일차 성 오브젝트가 로드된 후 스크립트를 안정적으로 진행하기 위함
local modelGate = folderCastle08:WaitForChild("Gate")     -- [의미/의도] 8일차 성 폴더 내부에서 "Gate" 성문 모델이 생성될 때까지 대기하여 가져옴 ➔ 성문에 타격 및 파괴 판정을 걸 대상 모델을 확실히 참조하기 위함
...
local function set_gate_color(color)                                                               -- [의미/의도] 성문 모델 내 모든 파트의 색을 지정한 color로 변경하는 함수 정의 ➔ 성문이 손상됨에 따라 시각적인 경고 색상으로 변화를 주기 위함
    for _, partGatePlank in ipairs(modelGate:GetDescendants()) do                                  -- [의미/의도] modelGate 모델 내부의 모든 하위 자식(Descendants)들을 순회하며 하나씩 partGatePlank에 담음 ➔ 성문을 구성하는 모든 나무판자 파트들을 누락 없이 제어하기 위함
        if partGatePlank:IsA("BasePart") then partGatePlank.BrickColor = BrickColor.new(color) end -- [의미/의도] 순회 중인 객체가 물리적인 파트(BasePart) 계열이라면 색상을 지정된 color로 변경 ➔ 성문 판자 파트들만 골라내어 깔끔하게 도색을 일괄 변경하기 위함
...
local function break_gate()                                                                                    -- [의미/의도] 성문 파괴를 실행하는 함수 정의 ➔ 성문의 고정을 해제하고 물리적으로 흩날리며 무너지는 파괴 효과를 주어 장엄하게 부서지는 모습을 연출하기 위함
    ...
    for _, partGatePlank in ipairs(modelGate:GetDescendants()) do                                              -- [의미/의도] 성문 모델 내부의 모든 하위 파트를 순회함 ➔ 5개의 판자 모두 물리 효과를 적용하여 붕괴시키기 위함
        if partGatePlank:IsA("BasePart") then                                                                  -- [의미/의도] 순회 객체가 BasePart 유형의 물리 파트인 경우 ➔ 물리 고정을 풀고 힘을 가할 수 있는 물리 오브젝트를 분류하기 위함
    ...
    end                                                                                                        -- [의미/의도] 붕괴 처리 반복문(for)의 종료 ➔ 모든 판자 파트에 붕괴 물리 적용 완료
end                                                                                                            -- [의미/의도] break_gate 함수의 종료 ➔ 성문 붕괴 함수 정의 완료

local function damage_gate(amount)                           -- [의미/의도] 성문에 피해를 주는 함수 정의 (피해량 amount를 매개변수로 받음) ➔ 입력된 만큼 내구도를 깎고 남은 체력에 따라 색 변경 및 파괴를 유도하기 위함
    if boolBroken then return end                            -- [의미/의도] 이미 문이 부서진 상태라면 함수 실행을 중단 ➔ 이미 부서진 문에 무의미한 데미지 처리가 반복되는 것을 차단하기 위함
    health -= amount                                         -- [의미/의도] 성문 체력(health)을 amount만큼 차감 ➔ 가해진 피해량만큼 문에 상처를 입힘
    if health <= 60 then set_gate_color("Bright orange") end -- [의미/의도] 체력이 60(절반) 이하로 떨어지면 성문 파트들을 주황색(Bright orange)으로 변경 ➔ 성문이 부서지기 일보 직전이라는 위급한 경고 신호를 시각적으로 주기 위함
...
for _, partGatePlank in ipairs(modelGate:GetDescendants()) do                       -- [의미/의도] 성문 모델 내부의 모든 하위 파트들을 순회함 ➔ 각각의 판자 파트마다 투사체 충돌(Touched) 이벤트를 일일이 연결해 주기 위함
    if partGatePlank:IsA("BasePart") then                                           -- [의미/의도] 파트가 BasePart 타입이면 ➔ 충돌 감지(Touched) 이벤트를 가질 수 있는 물리적 객체에만 연결을 시도하기 위함
        partGatePlank.Touched:Connect(function(partHit)                             -- [의미/의도] 개별 성문 판자에 무언가 닿았을 때(Touched) 충돌한 파트(partHit)를 매개변수로 함수 실행 ➔ 공격 투사체가 문에 부딪혔는지 실시간으로 검사하기 위함
            if partHit.Name == "TrainingArrow" or partHit.Name == "SiegeStone" then -- [의미/의도] 부딪힌 파트 이름이 화살("TrainingArrow") 또는 공성돌("SiegeStone")이라면 ➔ 아군 캐릭터의 몸이 부딪힌 것은 무시하고 오직 공격용 투사체에 맞았을 때만 피격 판정을 내리기 위함
        ...
        end)                                                                        -- [의미/의도] Touched 이벤트 콜백 함수의 종료 ➔ 해당 판자 충돌 이벤트 연결 완료
    end                                                                             -- [의미/의도] BasePart 판별 조건문의 종료 ➔ 조건 검사 완료
end                                                                                 -- [의미/의도] 전체 성문 파트 이벤트 연결 반복문(for)의 종료 ➔ 모든 성문 판자 파트에 개별 충돌 리스너 등록을 완료함
