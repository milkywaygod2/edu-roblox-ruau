...
local serviceStarterPack = game:GetService(common.enumServiceName.STARTER_PACK)                                        -- [의미/의도] StarterPack 서비스를 가져옴 ➔ 플레이어 접속 시 마법 지팡이 도구(Tool)를 인벤토리에 자동 배치해주기 위함
...
local remoteeventCastMagic = common.createOrReplaceInstance(common.enumObjectPhysicalType.REMOTE_EVENT, common.enumObjectLogicalType.CAST_MAGIC, serviceReplicatedStorage) -- [의미/의도] CastMagic RemoteEvent 대체 생성 ➔ 기존 마법 시전 리모트이벤트를 지우고 통신 채널을 새로 구축하기 위함

local toolMagicStaff = common.createOrReplaceInstance(common.enumObjectPhysicalType.TOOL, common.enumObjectLogicalType.MAGIC_STAFF, serviceStarterPack) -- [의미/의도] MagicStaff Tool 대체 생성 ➔ 기존 마법 지팡이를 정리하고 새로운 마법 무기를 초기화하기 위함
toolMagicStaff.ToolTip = "서버 판정 마법을 시전합니다"                                                                             -- [의미/의도] 도구 툴팁 설명을 작성 ➔ 플레이어에게 이 지팡이가 서버 연동식 마법 스킬용 장비임을 안내하기 위함

local partHandle = Instance.new(common.enumObjectPhysicalType.PART)                                                    -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 지팡이의 손잡이이자 겉보기 외형(Handle)을 만들기 위함
partHandle.Name = common.enumObjectLogicalType.RESERVED_HANDLE                                                         -- [의미/의도] 파트 이름을 반드시 "Handle"로 설정 ➔ 로블록스 도구 시스템이 캐릭터의 손 위치에 알아서 달라붙게 하기 위함
...
local folderMagicArena11 = common.createOrReplaceInstance(common.enumObjectPhysicalType.FOLDER, common.enumObjectLogicalType.MAGIC_ARENA11, serviceWorkspace) -- [의미/의도] MagicArena11 Folder 대체 생성 ➔ 기존 마법 아레나 폴더를 삭제하고 11일차 마법 연습장을 구성하기 위함

for index = 1, 6 do                                                                                                    -- [의미/의도] index 변수를 1부터 6까지 6번 반복 실행 ➔ 마법 광역 공격을 맞아줄 연습용 더미 6마리를 만들기 위함
    local modelPracticeDummy = Instance.new(common.enumObjectPhysicalType.MODEL)                                       -- [의미/의도] 새로운 모델(Model) 객체를 생성함 ➔ 캐릭터 역할을 할 파트와 Humanoid를 하나로 결합하기 위함
    modelPracticeDummy.Name = common.enumObjectLogicalType.MAGIC_DUMMY_PREFIX .. index                                 -- [의미/의도] 모델 이름을 인덱스 번호를 붙여 "1_MagicDummy" 등으로 설정 ➔ 각 더미를 고유 번호로 쉽게 구분하기 위함
    modelPracticeDummy.Parent = folderMagicArena11                                                                     -- [의미/의도] 모델의 부모를 MagicArena11 폴더로 지정 ➔ 11일차 아레나 폴더 내에 묶어 보관하기 위함

    local partHumanoidRoot = Instance.new(common.enumObjectPhysicalType.PART)                                          -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 캐릭터 중심 몸통(HumanoidRootPart) 역할을 할 파트를 만들기 위함
    partHumanoidRoot.Name = common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART                                   -- [의미/의도] 파트 이름을 반드시 "HumanoidRootPart"로 지정 ➔ Humanoid 시스템이 이 파트를 캐릭터의 물리 기준 좌표로 자동 연결하도록 하기 위함
    ...
    local humanoidPractice = Instance.new(common.enumObjectPhysicalType.HUMANOID)                                      -- [의미/의도] 새로운 Humanoid 객체를 생성함 ➔ 캐릭터에게 HP 및 TakeDamage 연산 등 생명체 기능을 부여하기 위함
