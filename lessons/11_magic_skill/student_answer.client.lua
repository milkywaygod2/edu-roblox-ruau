...
local eventCastMagic = svcReplicatedStorage:WaitForChild(eLogical.CAST_MAGIC)                  -- [의미/의도] ReplicatedStorage에서 "CastMagic" 리모트 이벤트가 준비될 때까지 대기하여 가져옴 ➔ 마우스 클릭 위치를 서버에 안전하게 원격 전송하기 위함
...
toolMagicStaff.Activated:Connect(function()                                                                   -- [의미/의도] 지팡이를 들고 클릭(Activated)했을 때 아래 함수를 실행 ➔ 마법 시전을 요청하기 위함
    ...
    eventCastMagic:FireServer(mousePlayer.Hit.Position)                                                          -- [의미/의도] 리모트 이벤트를 통해 마우스가 가리킨 3D 좌표(mousePlayer.Hit.Position)를 서버로 전송(FireServer) ➔ 마법이 시전될 조준 좌표를 서버에 실시간으로 요청하고, 실제 연산 처리를 위임하기 위함
