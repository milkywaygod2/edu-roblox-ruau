...
local servicePlayers = game:GetService(common.enumServiceName.PLAYERS)                                        -- [의미/의도] Players 서비스를 가져옴 ➔ 로컬 플레이어(LocalPlayer) 개체를 조회하기 위함
local serviceReplicatedStorage = game:GetService(common.enumServiceName.REPLICATED_STORAGE)                   -- [의미/의도] ReplicatedStorage 서비스를 가져옴 ➔ 서버와 통신할 리모트 이벤트를 공유 공간에서 찾기 위함
...
local remoteeventCastMagic = serviceReplicatedStorage:WaitForChild(common.enumObjectLogicalType.CAST_MAGIC)   -- [의미/의도] ReplicatedStorage에서 "CastMagic" 리모트 이벤트가 준비될 때까지 대기하여 가져옴 ➔ 마우스 클릭 위치를 서버에 안전하게 원격 전송하기 위함
