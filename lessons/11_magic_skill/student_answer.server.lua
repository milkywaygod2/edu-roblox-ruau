...
local servicePlayers = game:GetService(common.enumServiceName.PLAYERS)                                        -- [의미/의도] Players 서비스를 가져옴 ➔ 게임을 떠난 플레이어 정보를 감지하고 쿨타임 데이터를 청소하기 위함
local serviceReplicatedStorage = game:GetService(common.enumServiceName.REPLICATED_STORAGE)                   -- [의미/의도] ReplicatedStorage 서비스를 가져옴 ➔ 클라이언트와 통신하기 위해 공유된 통신망에서 리모트 이벤트를 참조하기 위함
local remoteeventCastMagic = serviceReplicatedStorage:WaitForChild(common.enumObjectLogicalType.CAST_MAGIC)   -- [의미/의도] ReplicatedStorage에서 "CastMagic" 리모트 이벤트가 생성될 때까지 대기하여 가져옴 ➔ 클라이언트의 마법 시전 요청 신호를 서버에서 대기하고 수신하기 위함
...
local function cast_magic(player, targetPosition)                                                             -- [의미/의도] 클라이언트의 요청을 받아 마법을 실행하는 서버 함수 정의 (요청 플레이어, 마우스 좌표 전달 받음) ➔ 보안이 유지되는 서버에서 실제 폭발을 소환하고 범위를 연산하기 위함
    ...
    local partHumanoidRoot = modelCharacter and modelCharacter:FindFirstChild(common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 캐릭터 몸통의 중심 파트(HumanoidRootPart)를 찾음 ➔ 사거리 검증을 위한 플레이어의 현재 실제 월드 좌표를 확인하기 위함
    ...
    local explosionMagic = Instance.new(common.enumObjectPhysicalType.EXPLOSION)                              -- [의미/의도] 새로운 폭발(Explosion) 객체를 생성함 ➔ 시각적으로 멋지게 터지는 폭풍 이펙트와 물리 충격을 보여주기 위함
    explosionMagic.Position = targetPosition                                                                  -- [의미/의도] 폭발이 일어날 위치를 클라이언트가 지정한 targetPosition으로 설정 ➔ 마우스를 조준하고 클릭한 정확한 좌표에 폭발 효과가 생기게 하기 위함
    explosionMagic.BlastRadius = RADIUS                                                                       -- [의미/의도] 폭발의 물리력 반경을 RADIUS(12)로 설정 ➔ 폭발 바람이 가해지는 가시 영역을 맞추기 위함
    explosionMagic.BlastPressure = 0                                                                          -- [의미/의도] 폭발의 물리적 밀쳐내는 압력(BlastPressure)을 0으로 설정 ➔ 로블록스 내장 폭발은 캐릭터를 사방으로 너무 강하게 날려 분해시키므로, 넉백을 차단하고 얌전히 이펙트만 쓰기 위함
    explosionMagic.DestroyJointRadiusPercent = 0                                                              -- [의미/의도] 폭발 반경 내 관절 파괴율(DestroyJointRadiusPercent)을 0%로 설정 ➔ 폭발에 닿은 캐릭터 관절이 조각조각 해체되어 즉사하는 기본 물리 성질을 완전히 제거하여 수동 데미지만 들어가도록 하기 위함
    explosionMagic.Parent = workspace                                                                         -- [의미/의도] 폭발 객체를 workspace의 자식으로 등록 ➔ 월드 상에 실제로 마법 폭발 불꽃이 번쩍이며 터지도록 이펙트를 실행하기 위함

    for _, object in ipairs(workspace:GetDescendants()) do                                                    -- [의미/의도] 게임 내의 모든 하위 오브젝트들을 전부 순회함 ➔ 폭발 반경 내에 들어온 타격 대상을 일일이 감지하기 위함
        if object:IsA(common.enumObjectPhysicalType.HUMANOID) then                                            -- [의미/의도] 순회 객체가 생명체 속성(Humanoid)인 경우 ➔ 폭발 범위 데미지를 입을 생명체를 식별하기 위함
            ...
            local partTargetRoot = modelTarget and modelTarget:FindFirstChild(common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 대상의 중심 몸통 파트(HumanoidRootPart)를 찾음 ➔ 폭발 중심점과의 실제 거리를 계산해내기 위함
...
remoteeventCastMagic.OnServerEvent:Connect(cast_magic)                                                        -- [의미/의도] 클라이언트가 리모트 이벤트를 전송(FireServer)했을 때 서버 측(OnServerEvent)에서 cast_magic 함수를 실행하도록 이벤트를 연동 ➔ 클라이언트의 시전 요청에 반응하여 서버에서 마법을 안전하게 가동하게 만들기 위함

servicePlayers.PlayerRemoving:Connect(function(player)                                                        -- [의미/의도] 플레이어가 서버에서 나갈 때(PlayerRemoving) 아래 함수를 호출 ➔ 퇴장한 플레이어의 데이터를 즉각 지워 서버 메모리를 보호하기 위함
