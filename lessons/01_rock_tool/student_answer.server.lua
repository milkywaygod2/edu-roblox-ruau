...
local serviceDebris = game:GetService(common.enumServiceName.DEBRIS)                                          -- [의미/의도] Debris 서비스를 가져옴 ➔ 생성된 투사체를 일정 시간 뒤 자동으로 삭제하기 위함
...
toolPracticeRock.Activated:Connect(function()                                                                 -- [의미/의도] 도구를 클릭(활성화)했을 때 아래 함수를 실행 ➔ 마우스 클릭 시 무기가 동작하도록 이벤트를 연결함
    ...
    local partHumanoidRoot = modelCharacter:FindFirstChild(common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 캐릭터 몸통의 중심 파트를 찾음 ➔ 발사 위치와 시선을 알아내기 위함
    ...
    local partRock = Instance.new(common.enumObjectPhysicalType.PART)                                         -- [의미/의도] 날아갈 파트(돌멩이)를 생성함 ➔ 물리적으로 날아가는 투사체 객체를 생성함
    partRock.Name = common.enumObjectLogicalType.THROWN_PRACTICE_ROCK                                         -- [의미/의도] 파트 이름을 "ThrownPracticeRock"으로 설정 ➔ 충돌 시 이름으로 구분할 수 있도록 함
    ...
    serviceDebris:AddItem(partRock, 5)                                                                        -- [의미/의도] 돌멩이 파트를 5초 뒤 자동 삭제하도록 예약 ➔ 빗나간 돌멩이가 맵에 쌓여 렉이 걸리지 않도록 방지함
    ...
    partRock.Touched:Connect(function(partHit)                                                                -- [의미/의도] 돌멩이가 어딘가에 부딪혔을 때(Touched) 아래 함수를 실행 ➔ 적과 부딪혔는지 감지하기 위함
        ...
        local modelTarget = partHit:FindFirstAncestorOfClass(common.enumObjectPhysicalType.MODEL)             -- [의미/의도] 충돌한 파트가 속한 최상위 모델을 찾음 ➔ 부딪힌 대상이 캐릭터(Model)인지 검사하기 위함
        local humanoidTarget = modelTarget and modelTarget:FindFirstChildOfClass(common.enumObjectPhysicalType.HUMANOID) -- [의미/의도] 그 모델 안에 생명체(Humanoid)가 있는지 찾음 ➔ 체력을 깎을 수 있는 대상인지 식별하기 위함
        local partTargetRoot = modelTarget and modelTarget:FindFirstChild(common.enumObjectLogicalType.RESERVED_HUMANOID_ROOT_PART) -- [의미/의도] 타격 대상의 중심 파트를 찾음 ➔ 넉백을 주기 위해 몸통을 찾기 위함
        if not humanoidTarget or modelTarget == modelCharacter then return end                                -- [의미/의도] 생명체가 없거나 자기 자신과 충돌한 경우 무시 ➔ 적군에게만 타격을 입히고 본인 자해를 막기 위함

        boolHitOnce = true                                                                                    -- [의미/의도] 타격 성공으로 변수 설정 ➔ 더 이상의 추가 타격을 차단함
        humanoidTarget:TakeDamage(DAMAGE)                                                                     -- [의미/의도] 대상을 DAMAGE만큼 공격 ➔ 무기 공격 데미지를 적용하기 위함
