...
local partPracticeBase = Instance.new(ePhysical.PART)                                       -- [의미/의도] 새 파트(블록)를 생성함 ➔ 학생들이 연습할 경기장의 바닥판을 만들기 위함
partPracticeBase.Name = eLogical.PRACTICE_BASE                                              -- [의미/의도] 파트 이름을 "PracticeBase"로 설정 ➔ 탐색기에서 바닥판 파트를 구분하기 위함
...
partPracticeBase.Parent = fldArena01                                                                             -- [의미/의도] 파트 부모를 1일차 폴더로 설정 ➔ 바닥판을 1일차 경기장 폴더 내부에 깔끔하게 정리하기 위함

for index = 1, 4 do                                                                                           -- [의미/의도] 1부터 4까지 반복 루프를 실행 ➔ 타격 연습용 더미 4개를 일정한 위치에 자동 생성하기 위함
    local modelPracticeDummy = Instance.new(ePhysical.MODEL)                                -- [의미/의도] 새 모델(Model)을 생성함 ➔ 더미의 파트와 인간 형태(Humanoid)를 하나로 묶기 위함
    modelPracticeDummy.Name = eLogical.PRACTICE_DUMMY_PREFIX .. index                       -- [의미/의도] 모델 이름을 번호에 맞게 설정 ➔ 각 더미를 고유 번호로 구분하기 위함
    modelPracticeDummy.Parent = fldArena01                                                                       -- [의미/의도] 모델 부모를 경기장 폴더로 설정 ➔ 더미를 경기장 폴더 내부에 깔끔하게 정렬하기 위함

    local partHumanoidRoot = Instance.new(ePhysical.PART)                                   -- [의미/의도] 새 파트를 생성함 ➔ 더미의 물리적 몸통 역할을 할 HumanoidRootPart를 만들기 위함
    partHumanoidRoot.Name = eLogical.RESERVED_HUMANOID_ROOT_PART                            -- [의미/의도] 파트 이름을 "HumanoidRootPart"로 설정 ➔ 캐릭터가 움직이고 타격받는 중심 물리 파트로 선언하기 위함
    ...
    local humPractice = Instance.new(ePhysical.HUMANOID)                                   -- [의미/의도] 새 Humanoid 객체를 생성함 ➔ 더미에게 체력과 데미지를 입을 수 있는 생명체 특성을 부여하기 위함
    humPractice.MaxHealth = 100                                                                                -- [의미/의도] 최대 체력을 100으로 설정 ➔ 표준 캐릭터 스펙의 최대 체력을 지정하기 위함
    humPractice.Health = 100                                                                                   -- [의미/의도] 현재 체력을 100으로 설정 ➔ 더미가 처음 생겨날 때 꽉 찬 체력을 갖게 하기 위함
    humPractice.Parent = modelPracticeDummy                                                                      -- [의미/의도] Humanoid의 부모를 더미 모델로 설정 ➔ 모델이 인간형 생명체로 작동되도록 최종 완성하기 위함
end

local toolPracticeRock = common.createOrReplaceInstance(ePhysical.TOOL, eLogical.PRACTICE_ROCK, svcStarterPack) -- [의미/의도] PracticeRock Tool 대체 생성 ➔ 기존 무기를 지우고 기본 지급용 무기를 초기화하기 위함
toolPracticeRock.ToolTip = "클릭하면 연습용 돌멩이를 던집니다"                                                                 -- [의미/의도] 말풍선 설명을 설정 ➔ 마우스를 올렸을 때 사용법 힌트를 띄우기 위함

local partHandle = Instance.new(ePhysical.PART)                                            -- [의미/의도] 새 파트를 생성함 ➔ 플레이어의 손에 직접 닿는 손잡이(Handle)를 구현하기 위함
partHandle.Name = eLogical.RESERVED_HANDLE                                                 -- [의미/의도] 파트 이름을 "Handle"로 설정 ➔ 로블록스 툴 시스템이 이 파트를 손잡이로 자동 인식하도록 약속된 이름을 쓰기 위함
