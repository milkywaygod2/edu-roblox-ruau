...
local folderOld = serviceWorkspace:FindFirstChild("Day02_CoverField") -- [의미/의도] Workspace 내에서 "Day02_CoverField" 이름을 가진 자식 객체를 찾음 ➔ 기존에 만들어진 폴더가 있는지 검사하기 위함
if folderOld then folderOld:Destroy() end                             -- [의미/의도] 기존 폴더가 존재한다면 제거함 ➔ 수업이 중복 실행되어 맵이 겹치는 현상을 방지하기 위함

local folderDay02CoverField = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder) 객체를 생성함 ➔ 2일차 수업에 필요한 파트들을 하나로 묶어 관리하기 위함
folderDay02CoverField.Name = "Day02_CoverField"      -- [의미/의도] 폴더의 이름을 "Day02_CoverField"로 설정 ➔ 게임 탐색기(Explorer)에서 폴더를 찾기 쉽게 지정함
folderDay02CoverField.Parent = serviceWorkspace      -- [의미/의도] 폴더의 부모를 Workspace로 설정 ➔ 폴더가 게임 세상에 실제로 추가되도록 함
...
partGridBase.Parent = folderDay02CoverField     -- [의미/의도] 바닥 파트를 Day02_CoverField 폴더의 자식으로 등록 ➔ 해당 폴더 안에서 깔끔하게 관리하기 위함
...
for index = 1, 5 do                                                  -- [의미/의도] index 변수를 1부터 5까지 증가시키며 반복함 ➔ 총 5개의 엄폐물 배치 기준 표시판(Marker)을 만들기 위함
    ...
    partCoverMarker.Name = "CoverMarker_" .. index                   -- [의미/의도] 마커의 이름을 번호를 붙여 "CoverMarker_1" 등으로 설정 ➔ 각각의 마커를 이름으로 쉽게 구분하기 위함
    ...
    partCoverMarker.Parent = folderDay02CoverField                   -- [의미/의도] 마커 파트를 Day02_CoverField 폴더의 자식으로 등록 ➔ 2일차 폴더 내부에서 엄폐물 마커들을 한데 모아 관리하기 위함
