...
-- --------------------------------------------------------------------------------
function module.applyInstanceProperties(instanceTarget, tblProperties) -- [의미/의도] 인스턴스 속성 묶음 적용 함수 정의 ➔ Size/Position/Material 같은 반복 속성 대입을 한곳에서 처리하기 위함
...
function module.resetNamedInstance(strClassName, strName, instanceParent, tblProperties) -- [의미/의도] 이름 있는 인스턴스 삭제 후 재생성 함수 정의 ➔ 수업장을 깨끗하게 초기화해야 하는 예외 상황에 사용하기 위함
	...
	module.applyInstanceProperties(instanceNew, tblProperties) -- [의미/의도] 전달된 속성 묶음을 적용 ➔ 생성 직후 필요한 설정을 한 번에 반영하기 위함
...
function module.ensureNamedInstance(strClassName, strName, instanceParent, tblProperties) -- [의미/의도] 이름 있는 인스턴스 보장 함수 정의 ➔ 누적형 전초기지 월드에서 기존 오브젝트를 지우지 않고 재사용하기 위함
	...
	module.applyInstanceProperties(instanceTarget, tblProperties) -- [의미/의도] 속성 보정 ➔ 기존 객체를 유지하되 크기/위치/재질 같은 기준값은 수업 표준으로 맞추기 위함
...
function module.ensureStaticPart(strName, instanceParent, tblProperties) -- [의미/의도] 고정 Part 보장 함수 정의 ➔ 바닥/버튼/마커가 회차마다 삭제되지 않고 누적 유지되도록 하기 위함
	...
	return module.ensureNamedInstance(EngineNames.eEnginePhysicalType.PART, strName, instanceParent, tblPartProperties) -- [의미/의도] Part 보장 실행 ➔ 기존 Part는 유지하고 필요한 속성만 보정하기 위함
end
...
function module.ensureClickDetector(partParent, intMaxActivationDistance) -- [의미/의도] ClickDetector 보장 함수 정의 ➔ 버튼 setup 재실행 시 감지기가 중복 생성되지 않게 하기 위함
	local clickDetector = partParent:FindFirstChildOfClass(EngineNames.eEnginePhysicalType.CLICK_DETECTOR) -- [의미/의도] 기존 ClickDetector 검색 ➔ 이미 연결된 클릭 컴포넌트를 재사용하기 위함
	if not clickDetector then -- [의미/의도] 클릭 감지기가 없다면 ➔ 버튼 기능을 새로 추가하기 위함
		clickDetector = Instance.new(EngineNames.eEnginePhysicalType.CLICK_DETECTOR) -- [의미/의도] 새 ClickDetector 생성 ➔ Part를 마우스로 클릭 가능한 버튼으로 만들기 위함
...
function module.ensureToolWithHandle(strToolName, strToolTip, instanceParent, tblHandleProperties) -- [의미/의도] Tool과 Handle 보장 함수 정의 ➔ 학생 스크립트나 커스터마이즈가 붙은 Tool을 삭제하지 않고 규격만 보정하기 위함
	local toolTarget = module.ensureNamedInstance(EngineNames.eEnginePhysicalType.TOOL, strToolName, instanceParent) -- [의미/의도] Tool 보장 ➔ 기존 장비와 내부 Script를 유지하면서 없으면 생성하기 위함
	...
	local partHandle = module.ensureNamedInstance(EngineNames.eEnginePhysicalType.PART, EngineNames.eEngineLogicalType.RESERVED_HANDLE, toolTarget, tblHandleProperties) -- [의미/의도] Handle Part 보장 ➔ Tool이 캐릭터 손에 부착될 기준 파트를 유지 또는 생성하기 위함
...
function module.ensureHumanoidTarget(strName, instanceParent, tblRootPartProperties, tblHumanoidProperties) -- [의미/의도] Humanoid 목표물 보장 함수 정의 ➔ 전초기지 코어/가드가 회차 재실행 때 삭제되지 않고 기준만 보정되게 하기 위함
	local modelTarget = module.ensureNamedInstance(EngineNames.eEnginePhysicalType.MODEL, strName, instanceParent) -- [의미/의도] 목표물 Model 보장 ➔ 몸통 파트와 Humanoid를 하나의 피해 가능 오브젝트로 묶기 위함
	local partHumanoidRoot = module.ensureStaticPart(EngineNames.eEngineLogicalType.RESERVED_HUMANOID_ROOT_PART, modelTarget, tblRootPartProperties) -- [의미/의도] HumanoidRootPart 보장 ➔ 데미지 판정과 위치 기준이 되는 중심 파트를 유지 또는 생성하기 위함
	local humTarget = module.ensureNamedInstance(EngineNames.eEnginePhysicalType.HUMANOID, EngineNames.eEnginePhysicalType.HUMANOID, modelTarget, tblHumanoidProperties) -- [의미/의도] Humanoid 보장 ➔ 목표물이 체력과 피해 처리를 갖게 하기 위함
...
function module.ensureOutpostBattleWorld(svcWorkspace) -- [의미/의도] 누적 전초기지 공방전 월드 보장 함수 정의 ➔ 어떤 회차부터 실행해도 같은 5v5 목표전 전장 구조가 유지되게 하기 위함
	local ePhysical = EngineNames.eEnginePhysicalType
	local eLogical = EngineNames.eEngineLogicalType

	local fldOutpostBattleWorld = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.OUTPOST_BATTLE_WORLD, svcWorkspace) -- [의미/의도] 전초기지 공방전 루트 보장 ➔ 모든 회차 콘텐츠를 하나의 월드 아래에 누적하기 위함
	local fldBattlefield = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.BATTLEFIELD, fldOutpostBattleWorld) -- [의미/의도] 전투 공간 폴더 보장 ➔ 플레이 가능한 중심 전장을 유지하기 위함
	local fldBuildArea = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.BUILD_AREA, fldOutpostBattleWorld) -- [의미/의도] 건설 영역 폴더 보장 ➔ 방벽과 학생 건축물을 누적하기 위함
	local fldObjectiveArea = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.OBJECTIVE_AREA, fldOutpostBattleWorld) -- [의미/의도] 목표물 영역 폴더 보장 ➔ 코어/가드/표적을 누적하기 위함
	local fldItemSpawnArea = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.ITEM_SPAWN_AREA, fldOutpostBattleWorld) -- [의미/의도] 아이템 스폰 영역 보장 ➔ 파밍 장비를 전장 곳곳에 유지하기 위함
	local fldFortification = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.FORTIFICATION, fldOutpostBattleWorld) -- [의미/의도] 방어 구조물 영역 보장 ➔ 문과 벽을 기지 방어 구조로 관리하기 위함
	local fldSiegeEngine = module.ensureNamedInstance(ePhysical.FOLDER, eLogical.SIEGE_ENGINE, fldOutpostBattleWorld) -- [의미/의도] 중장비 폴더 보장 ➔ 발사 버튼과 기준점을 누적 관리하기 위함
	local partBattlefieldBase = module.ensureStaticPart(eLogical.BATTLEFIELD_BASE, fldBattlefield, { -- [의미/의도] 공통 전장 바닥 보장 ➔ 1회차부터 12회차까지 같은 플레이 공간을 유지하기 위함
...
function module.waitForOutpostBattleWorld(svcWorkspace) -- [의미/의도] 누적 전초기지 공방전 월드 대기 함수 정의 ➔ 학생/시스템 스크립트가 선생님 setup이 만든 공통 구조를 안전하게 참조하기 위함
	local eLogical = EngineNames.eEngineLogicalType
...
function module.markRuntimeInstalled(instanceTarget, strAttributeKey) -- [의미/의도] 런타임 시스템 중복 설치 방지 함수 정의 ➔ 같은 버튼/Tool에 이벤트가 여러 번 연결되는 것을 막기 위함
...
function module.removeLuaSourceDescendants(instanceTarget)
