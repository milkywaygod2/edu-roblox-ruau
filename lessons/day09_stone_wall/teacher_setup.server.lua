...
local folderOld = serviceWorkspace:FindFirstChild("Day09_StoneWall") -- [의미/의도] Workspace에서 기존 "Day09_StoneWall" 폴더가 존재하는지 확인 ➔ 중복으로 생성되는 것을 감지하기 위함
...
local folderDay09StoneWall = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder) 객체를 생성함 ➔ 9일차 실습에서 제어할 성벽 섹션 모델들을 하나로 그룹화하여 관리하기 위함
folderDay09StoneWall.Name = "Day09_StoneWall"       -- [의미/의도] 폴더 이름을 "Day09_StoneWall"로 지정 ➔ 탐색기에서 9일차 석조 성벽 프로젝트 영역임을 파악하기 위함
folderDay09StoneWall.Parent = serviceWorkspace      -- [의미/의도] 폴더 부모를 Workspace로 설정 ➔ 폴더가 게임 세상에 반영되도록 하기 위함
...
for section = 1, 5 do                                 -- [의미/의도] section 변수를 1부터 5 till 5까지 증가시키며 반복함 ➔ 성벽을 5개의 부분 섹션(Section)으로 나누어 독립적인 파괴가 가능하도록 구조화하기 위함
    ...
    modelWallSection.Name = "WallSection_" .. section -- [의미/의도] 모델 이름을 인덱스를 붙여 "WallSection_1" 등으로 설정 ➔ 각각의 성벽 구획을 고유 번호로 쉽게 구분하고 독립 처리하기 위함
    modelWallSection.Parent = folderDay09StoneWall    -- [의미/의도] 성벽 섹션 모델 부모를 Day09_StoneWall 폴더로 지정 ➔ 9일차 성벽 폴더 내에 가지런히 소속시키기 위함
