...
local folderDay10SiegeEngine = workspace:WaitForChild("Day10_SiegeEngine")   -- [의미/의도] Workspace에서 "Day10_SiegeEngine" 폴더가 생성될 때까지 대기하여 가져옴 ➔ 10일차 투석기 오브젝트들이 완전히 로드된 후 스크립트를 수행하기 위함
local partLaunchButton = folderDay10SiegeEngine:WaitForChild("LaunchButton") -- [의미/의도] 10일차 폴더 내부에서 "LaunchButton" 파트가 생성될 때까지 대기하여 가져옴 ➔ 발사를 작동시킬 입력 장치(버튼)를 정확하게 셋업하기 위함
local partLaunchPoint = folderDay10SiegeEngine:WaitForChild("LaunchPoint")   -- [의미/의도] 10일차 폴더 내부에서 "LaunchPoint" 파트가 생성될 때까지 대기하여 가져옴 ➔ 공성 돌이 소환되는 공중 시작 위치 좌표를 가져오기 위함
local partTargetPoint = folderDay10SiegeEngine:WaitForChild("TargetPoint")   -- [의미/의도] 10일차 폴더 내부에서 "TargetPoint" 파트가 생성될 때까지 대기하여 가져옴 ➔ 투사체가 날아가 부딪힐 정면 목표물 위치 좌표를 가져오기 위함
