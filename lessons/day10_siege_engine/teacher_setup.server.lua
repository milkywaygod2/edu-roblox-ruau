...
local folderOld = serviceWorkspace:FindFirstChild("Day10_SiegeEngine") -- [의미/의도] Workspace 내에서 기존 "Day10_SiegeEngine" 폴더가 존재하는지 확인 ➔ 중복으로 지급되는 리소스가 있는지 찾기 위함
if folderOld then folderOld:Destroy() end                              -- [의미/의도] 기존 공성 병기 폴더가 존재한다면 제거 ➔ 10일차 셋업 재실행 시 버튼과 발사 위치가 맵에 중복으로 겹쳐 생성되는 것을 방지하기 위함

local folderDay10SiegeEngine = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder) 객체를 생성함 ➔ 10일차 실습에서 다룰 발사 버튼, 발사 원점, 표적지를 하나로 그룹화하여 관리하기 위함
folderDay10SiegeEngine.Name = "Day10_SiegeEngine"     -- [의미/의도] 폴더 이름을 "Day10_SiegeEngine"으로 설정 ➔ 탐색기에서 10일차 공성 병기 관련 리소스임을 쉽게 알아볼 수 있게 하기 위함
folderDay10SiegeEngine.Parent = serviceWorkspace      -- [의미/의도] 폴더 부모를 Workspace로 설정 ➔ 폴더가 게임 월드에 실존하게 하여 플레이어가 상호작용하도록 하기 위함
...
partLaunchButton.Parent = folderDay10SiegeEngine            -- [의미/의도] 버튼 파트를 Day10_SiegeEngine 폴더의 자식으로 등록 ➔ 10일차 폴더 내부에서 버튼을 관리하기 위함

local clickdetectorButton = Instance.new("ClickDetector") -- [의미/의도] 새로운 클릭 감지기(ClickDetector) 컴포넌트를 생성함 ➔ 마우스로 이 파트를 누를 수 있는 인터랙션 기능을 부여하기 위함
...
local partLaunchPoint = Instance.new("Part")     -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 공성 돌이 발사되어 출발하는 물리적 위치 마커(LaunchPoint)를 만들기 위함
partLaunchPoint.Name = "LaunchPoint"             -- [의미/의도] 파트 이름을 "LaunchPoint"로 지정 ➔ 스크립트가 돌 탄환을 스폰시킬 지점으로 참조하게 하기 위함
...
partLaunchPoint.Parent = folderDay10SiegeEngine  -- [의미/의도] 마커 파트를 Day10_SiegeEngine 폴더의 자식으로 등록 ➔ 10일차 폴더 내에서 관리하기 위함

local partTargetPoint = Instance.new("Part")              -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 탄환이 날아가 떨어질 목표물 위치 마커(TargetPoint)를 만들기 위함
partTargetPoint.Name = "TargetPoint"                      -- [의미/의도] 파트 이름을 "TargetPoint"로 설정 ➔ 탄환 발사 궤적을 계산할 종점 타겟 좌표로 쓰기 위함
...
partTargetPoint.Parent = folderDay10SiegeEngine           -- [의미/의도] 타겟 파트를 Day10_SiegeEngine 폴더의 자식으로 등록 ➔ 10일차 폴더 내에 함께 묶어 관리하기 위함
