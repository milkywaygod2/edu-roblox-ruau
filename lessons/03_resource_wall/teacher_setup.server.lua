...
partBuildButton.Name = eLogical.BUILD_BUTTON                                                -- [의미/의도] 파트 이름을 "BuildButton"으로 설정 ➔ 탐색기에서 방벽 설치 버튼임을 구분하기 위함
...
partBuildButton.Parent = fldResourceWall03                                                                        -- [의미/의도] 버튼 파트를 ResourceWall03 폴더의 자식으로 등록 ➔ 3일차 폴더 내부에서 관리하기 위함

local clickButton = Instance.new(ePhysical.CLICK_DETECTOR)                                  -- [의미/의도] 새로운 클릭 감지기(ClickDetector) 컴포넌트를 생성함 ➔ 플레이어가 버튼 파트를 마우스로 클릭할 수 있게 기능적으로 연결하기 위함
clickButton.MaxActivationDistance = 24                                                                          -- [의미/의도] 클릭할 수 있는 최대 반경을 24칸으로 제한 ➔ 너무 멀리서 부당하게 버튼을 눌러 소환하는 플레이를 제한하기 위함
clickButton.Parent = partBuildButton                                                                              -- [의미/의도] 클릭 감지기의 부모를 앞서 만든 버튼 파트로 설정 ➔ 버튼 파트에 마우스 클릭 감지 기능을 활성화하기 위함

local partWallSpawn = Instance.new(ePhysical.PART)                                          -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 방벽이 실제로 생성될 예상 위치를 나타낼 마커(WallSpawn)를 만들기 위함
partWallSpawn.Name = eLogical.WALL_SPAWN                                                    -- [의미/의도] 파트 이름을 "WallSpawn"으로 설정 ➔ 방벽 소환판임을 구분하기 위함
...
partWallSpawn.Parent = fldResourceWall03                                                                          -- [의미/의도] 마커 파트를 ResourceWall03 폴더의 자식으로 등록 ➔ 3일차 폴더 내부에서 함께 관리하기 위함
