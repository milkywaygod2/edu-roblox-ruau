...
-- 역할: teacher_setup.server.lua, 선생님 전용 수업 준비 코드입니다.
-- 편집 위치: Roblox Studio에서 Explorer, Properties, Output 창을 켜고 작업합니다.
-- 붙여넣기 위치: ServerScriptService > Script 이름 ranged_weapon_TeacherSetup_05
...
local folderOldRange = serviceWorkspace:FindFirstChild("TargetRange05") -- [의미/의도] Workspace에서 기존 "TargetRange05" 폴더가 있는지 확인 ➔ 기존 리소스를 제거하기 위함
if folderOldRange then folderOldRange:Destroy() end                     -- [의미/의도] 기존 과녁 연습장 폴더가 존재하면 삭제 ➔ 스크립트 재실행 시 중복 생성을 막고 맵을 깨끗하게 정리하기 위함

local folderTargetRange05 = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder) 객체를 생성함 ➔ 5일차 실습에서 쏠 과녁 판들을 묶어서 편하게 관리하기 위함
folderTargetRange05.Name = "TargetRange05"         -- [의미/의도] 폴더 이름을 "TargetRange05"로 설정 ➔ 탐색기(Explorer)에서 5일차 전용 영역임을 쉽게 알아보기 위함
folderTargetRange05.Parent = serviceWorkspace      -- [의미/의도] 폴더 부모를 Workspace로 설정 ➔ 폴더가 게임 세상에 반영되도록 만들기 위함
