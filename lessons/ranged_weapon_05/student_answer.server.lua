...
-- 수업: ranged_weapon_05 - 원거리 무기와 포물선
...
local serviceDebris = game:GetService("Debris") -- [의미/의도] Debris 서비스를 가져옴 ➔ 월드에 발사된 화살이 무한히 남아 서버에 부하를 주지 않도록 일정 시간 후 자동 소멸시키기 위함
...
toolTrainingBow.Activated:Connect(function() -- [의미/의도] 활을 클릭(활성화)했을 때 아래 함수를 실행 ➔ 활 쏘기 발사 로직을 연동하기 위함
    ...
    local partHumanoidRoot = modelCharacter:FindFirstChild("HumanoidRootPart") -- [의미/의도] 캐릭터 몸통의 중심 파트(HumanoidRootPart)를 찾음 ➔ 화살 발사의 기준점과 바라보는 시선 방향(CFrame)을 구하기 위함
    ...
    local partArrow = Instance.new("Part")                                                                 -- [의미/의도] 새로운 파트(Part) 객체를 생성함 ➔ 물리적으로 날아가는 투사체인 화살 객체를 만들기 위함
    partArrow.Name = "TrainingArrow"                                                                       -- [의미/의도] 화살 파트 이름을 "TrainingArrow"로 설정 ➔ 충돌 시 이름 비교를 통해 화살인지 식별할 수 있도록 위함
    ...
    partArrow.Touched:Connect(function(partHit)                                                    -- [의미/의도] 날아가는 화살이 어딘가에 충돌(Touched)했을 때 아래 함수를 실행 ➔ 충돌된 물체를 검사하고 타격 효과를 주거나 화살을 처리하기 위함
        ...
        local humanoidTarget = partHit.Parent:FindFirstChildOfClass("Humanoid")                    -- [의미/의도] 부딪힌 대상 부모에서 생명체 속성(Humanoid)을 찾음 ➔ 체력이 존재하는 플레이어나 몬스터인지 판정하기 위함
        if humanoidTarget then humanoidTarget:TakeDamage(DAMAGE) end                               -- [의미/의도] 생명체가 맞으면 DAMAGE(18)만큼 체력을 깎음 ➔ 원거리 공격 피해를 정량적으로 적용하기 위함
        if partHit.Name:match("Target") then partHit.BrickColor = BrickColor.new("Lime green") end -- [의미/의도] 맞은 파트 이름에 "Target" 단어가 포함되어 있다면 파트 색상을 라임색(Lime green)으로 변경 ➔ 과녁판을 맞혔을 때 명중했음을 초록색으로 시각적으로 명확히 피드백해주기 위함
