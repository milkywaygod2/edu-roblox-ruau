...
-- 수업: melee_weapon_04 - 일반 무기와 디바운스
...
-- 역할: teacher_setup.server.lua, 선생님 전용 수업 준비 코드입니다.
-- 편집 위치: Roblox Studio에서 Explorer, Properties, Output 창을 켜고 작업합니다.
-- 붙여넣기 위치: ServerScriptService > Script 이름 melee_weapon_TeacherSetup_04
...
local folderOldDummies = serviceWorkspace:FindFirstChild("Dummies04") -- [의미/의도] Workspace에서 기존 "Dummies04" 폴더가 존재하는지 확인 ➔ 중복 생성을 검출하기 위함
...
local folderDummies04 = Instance.new("Folder") -- [의미/의도] 새로운 폴더(Folder)를 생성함 ➔ 4일차 연습용 쿨타임 더미들을 그룹화하여 관리하기 위함
folderDummies04.Name = "Dummies04"             -- [의미/의도] 폴더 이름을 "Dummies04"로 지정 ➔ 탐색기에서 4일차 전용 더미 폴더임을 식별하게 위함
folderDummies04.Parent = serviceWorkspace      -- [의미/의도] 폴더 부모를 Workspace로 설정 ➔ 월드 상에 더미 폴더가 존재하도록 만들기 위함
